...
class AddTicketsSchema extends Migration
{
    ...
    {
        Schema::create('ticket_categories', function ($table): void {
            $table->increments('id');
            $table->text('name');
        ...
        Schema::create('ticket_relations', function ($table): void {
            $table->increments('id');
        ...
        Schema::create('ticket_templates', function ($table): void {
            $table->increments('id');
        ...
        Schema::create('ticket_comments', function ($table): void {
            $table->increments('id');
            $table->text('description');
            $table->string('contact_key', 255);
            $table->unsignedInteger('user_id');
        ...
        Schema::table('activities', function ($table): void {
            ...
            $table->index(['ticket_id', 'company_id']);
        });


        Schema::create('ticket_invitations', function ($table): void {
            $table->increments('id');
            $table->unsignedInteger('company_id');
            $table->unsignedInteger('user_id');
        ...
        Schema::create('lookup_ticket_invitations', function ($table): void {
            $table->increments('id');
        ...
        Schema::create('account_ticket_settings', function ($table): void {
            $table->increments('id');
        ...
        foreach ($companys as $company) {
            if (!$company->account_ticket_settings) {
                /* Create account_ticket_settings record for company */

                $user = $company->users()->where('public_id', '=', 0)->orWhereNull('public_id')->first();

                if ($user) {
                    $companyTicketSettings = new AccountTicketSettings();
                    $companyTicketSettings->ticket_master_id = $user->id;

                    $company->account_ticket_settings()->save($companyTicketSettings);
                } else {
                    Log::error('company ' . $company->id . 'does not have a owner user');
                }
            }
        }
    }
    ...
    public function down(): void
    {
        ...
        if (!Utils::isNinja()) {
            Schema::table('activities', function ($table): void {
                $table->dropIndex(['contact_id', 'company_id']);
                $table->dropIndex(['payment_id', 'company_id']);
                $table->dropIndex(['invitation_id', 'company_id']);
                $table->dropIndex(['user_id', 'company_id']);
                $table->dropIndex(['invoice_id', 'company_id']);
                $table->dropIndex(['client_id', 'company_id']);
            });
        ...
        if (Schema::hasColumn('activities', 'ticket_id')) {
            Schema::table('activities', function ($table): void {
                $table->dropColumn('ticket_id');
                $table->dropIndex(['ticket_id', 'company_id']);
            });
        }
