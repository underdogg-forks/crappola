            ...
            $('#add-ach').click(function(e) {
                ...
                WePay.bank_account.create({
                    ...
                    if(dataObj.bank_account_id) {
                        ...
                        $('#add-ach').after($('<div id="wepay-error" style="margin-top:20px" class="relative px-3 py-3 mb-4 border rounded text-red-darker border-red-dark bg-red-lighter"></div>').text(dataObj.error_description));
                    }
                });
            });
        });
...
<div class="modal opacity-0" id="completeVerificationModal" tabindex="-1" role="dialog" aria-labelledby="completeVerificationModalLabel" aria-hidden="true">
    <div class="modal-dialog" style="min-width:150px">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="mb-4">
                    <label for="verification1" class="control-label sm:w-2/5 pr-4 pl-4">{{ trans('texts.verification_amount1') }}</label>
                    <div class="sm:w-1/4 pr-4 pl-4">
                        <div class="relative flex items-stretch w-full">
                            <span class="py-1 px-2 mb-1 text-base font-normal leading-normal text-grey-darkest text-center bg-grey-light border border-4 border-grey-lighter rounded"><span class="payment_method_currenct_symbol"></span>0.</span>
                            <input type="number" min="0" max="99" required class="block appearance-none w-full py-1 px-2 mb-1 text-base leading-normal bg-white text-grey-darker border border-grey rounded" id="verification1" name="verification1">
                        </div>
                    </div>
                </div>
                <div class="mb-4">
                    <label for="verification2" class="control-label sm:w-2/5 pr-4 pl-4">{{ trans('texts.verification_amount2') }}</label>
                    <div class="sm:w-1/4 pr-4 pl-4">
                        <div class="relative flex items-stretch w-full">
                            <span class="py-1 px-2 mb-1 text-base font-normal leading-normal text-grey-darkest text-center bg-grey-light border border-4 border-grey-lighter rounded"><span class="payment_method_currenct_symbol"></span>0.</span>
                            <input type="number" min="0" max="99" required class="block appearance-none w-full py-1 px-2 mb-1 text-base leading-normal bg-white text-grey-darker border border-grey rounded" id="verification2" name="verification2">
                        </div>
                    </div>
                </div>
            </div>
            <div class="modal-footer" style="margin-top: 2px">
                ...
                <button type="submit" class="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap py-2 px-4 rounded text-base leading-normal no-underline text-blue-lightest bg-blue hover:bg-blue-light">{{ trans('texts.complete_verification') }}</button>
            </div>
            {!! Former::close() !!}
        </div>
    </div>
...
<div class="modal opacity-0" id="removePaymentMethodModal" tabindex="-1" role="dialog" aria-labelledby="removePaymentMethodModalLabel" aria-hidden="true">
    <div class="modal-dialog" style="min-width:150px">
        <div class="modal-content">
            ...
            <div class="modal-footer" style="margin-top: 2px">
                <button type="button" class="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap py-2 px-4 rounded text-base leading-normal no-underline btn-default" data-dismiss="modal">{{ trans('texts.cancel') }}</button>
                <button type="submit" class="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap py-2 px-4 rounded text-base leading-normal no-underline text-blue-lightest bg-blue hover:bg-blue-light">{{ trans('texts.remove') }}</button>
            </div>
            {!! Former::close() !!}
        </div>
    </div>
