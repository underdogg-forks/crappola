...
<head>
    ...
    <!-- Version: 4.5.19 -->
            ...
            <title>Payments | Invoice Ninja</title>
    ...
    <link rel="canonical" href="https://app.invoiceninja.com/payments"/>
...
<div id="wrapper" class=' toggled-right'>
    ...
    <a href="/reports"
        ...
        Reports
            ...
            <a type="button" class="btn btn-default btn-sm pull-right" title="User Guide: Payments &gt; List Payments"
            href="https://invoice-ninja.readthedocs.io/en/latest/payments.html#list-payments" target="_blank">
...
<script type="text/javascript">
    ...
    <form accept-charset="utf-8" class="form-horizontal listForm_payment" method="POST" action="/payments/bulk">

<div style="display:none">
	<div class="form-group"><label for="action_payment" class="control-label col-lg-4 col-sm-4">Action</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="action_payment" type="text" name="action"></div></div>
    <div class="form-group"><label for="public_id_payment" class="control-label col-lg-4 col-sm-4">public id</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="public_id_payment" type="text" name="public_id"></div></div>
...
<div class="pull-left">
	...
	<div class='btn-group'><button class='btn btn-default archive' type='button'>Archive</button><button class='btn btn-default dropdown-toggle archive' data-toggle='dropdown' type='button'><span class='caret'></span></button><ul class='dropdown-menu' role='menu' aria-labelledby='dLabel'><li><a href='javascript:submitForm_payment("archive")'>Archive Payment</a></li><li><a href='javascript:submitForm_payment("delete")'>Delete Payment</a></li></ul></div>

	&nbsp;
	<span id="statusWrapper_payment" style="display:none">
		<select class="form-control" style="width: 220px" id="statuses_payment" multiple="true">
	...
	&nbsp;
	<span class="well well-sm" id="sum_column_payment" style="display:none;padding-left:12px;padding-right:12px;"></span>
</div>

<div id="top_right_buttons" class="pull-right">
	<input id="tableFilter_payment" type="text" style="width:180px;margin-right:17px;background-color: white !important"
        class="form-control pull-left" placeholder="Filter" value=""/>

                
	
	    	<a class='btn btn-primary' href='http://invninjv1.local/payments/create'>Enter Payment <span class='glyphicon glyphicon-plus-sign'></span></a>
	
</div>


<table class="table table-striped data-table yAYtMUFr">
    <colgroup>
                <col class="con0" />
                <col class="con1" />
                <col class="con2" />
    ...
    <tr>
                ...
                <th align="center" valign="middle" class="head9"
                    >
...
<script type="text/javascript">
    ...
    function refreshDatatable_payments() {
        window['dataTable_payments'].api().ajax.reload();
    }

    function load_yAYtMUFr() {
        window['dataTable_payments'] = jQuery('.yAYtMUFr').dataTable({
            "stateSave": true,
            "stateDuration": 0,
            ...
            "aoColumnDefs": [
                                // Disable sorting on the first column
                {
                    ...
                    'aTargets': []
                        ...
                        "sPaginationType": "bootstrap",
                        "bProcessing": false,
                        "sAjaxSource": "http:\/\/invninjv1.local\/api\/payments",
                        "bServerSide": true,
                        "aaSorting": [[7,"desc"]],
                                    "fnDrawCallback": function(oSettings) {
                                    if (window.onDatatableReady_payments) {
                        window.onDatatableReady_payments();
...
<style type="text/css">
			...
			.listForm_payment table.dataTable td:nth-child(7) {
			text-align: right;
		}
	
			.listForm_payment table.dataTable td:nth-child(9) {
			text-align: center;
		}
...
<script type="text/javascript">
	...
	var submittedForm;
	function submitForm_payment(action, id) {
		...
		if (id) {
			$('#public_id_payment').val(id);
		}

		if (action == 'delete' || action == 'emailInvoice') {
	        sweetConfirm(function() {
	            $('#action_payment').val(action);
	    		$('form.listForm_payment').submit();
	        });
		} else {
			$('#action_payment').val(action);
			$('form.listForm_payment').submit();
	...
	$(function() {
	    ...
	    function filterTable_payment(val) {
	        if (val == tableFilter) {
	            return;
	        }
	        tableFilter = val;
			var oTable0 = $('.listForm_payment .data-table').dataTable();
	        oTable0.fnFilter(val);
	    }

	    $('#tableFilter_payment').on('keyup', function(){
	        if (searchTimeout) {
	            window.clearTimeout(searchTimeout);
	        }
	        searchTimeout = setTimeout(function() {
	            filterTable_payment($('#tableFilter_payment').val());
	        }, 500);
	    })

	    if ($('#tableFilter_payment').val()) {
	        filterTable_payment($('#tableFilter_payment').val());
	    }

		$('.listForm_payment .head0').click(function(event) {
			if (event.target.type !== 'checkbox') {
				$('.listForm_payment .head0 input[type=checkbox]').click();
			}
		});

		// Enable/disable bulk action buttons
	    window.onDatatableReady_payments = function() {
	        $(':checkbox').click(function() {
	            setBulkActionsEnabled_payment();
							changeSumLabel();
	        });

	        $('.listForm_payment tbody tr').unbind('click').click(function(event) {
	    ...
	    $('.listForm_payment .archive, .invoice').prop('disabled', true);
	    $('.listForm_payment .archive:not(.dropdown-toggle)').click(function() {
	        submitForm_payment('archive');
	    });

	    $('.listForm_payment .selectAll').click(function() {
	        $(this).closest('table').find(':checkbox:not(:disabled)').prop('checked', this.checked);
	    });

	    function setBulkActionsEnabled_payment() {
	        var buttonLabel = "Archive";
	        var count = $('.listForm_payment tbody :checkbox:checked').length;
	        $('.listForm_payment button.archive, .listForm_payment button.invoice').prop('disabled', !count);
	        if (count) {
	            buttonLabel += ' (' + count + ')';
	        }
	        $('.listForm_payment button.archive').not('.dropdown-toggle').text(buttonLabel);
	    }
			...
			function changeSumLabel() {
				var dTable = $('.listForm_payment .data-table').DataTable();
					...
					if (sum) {
						$('#sum_column_payment').show().text("Total: " + sum)
					} else {
						$('#sum_column_payment').hide();
					}

				 			}

		// Setup state/status filter
		$('#statuses_payment').select2({
			placeholder: "Status",
			//allowClear: true,
			templateSelection: function(data, container) {
			  ...
			  .trigger('change')
		  .on('change', function() {
			var filter = $('#statuses_payment').val();
			if (filter) {
				filter = filter.join(',');
			} else {
				filter = '';
			}
			var url = 'http://invninjv1.local/set_entity_filter/payment' + '/' + filter;
	        $.get(url, function(data) {
	            refreshDatatable_payments();
	        })
		}).maximizeSelect2Height();

		$('#statusWrapper_payment').show();
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a class="btn btn-primary" href="https://invoice-ninja.readthedocs.io/en/latest/payments.html#list-payments" target="_blank">User Guide</a>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        if ('http://invninjv1.local/payments' != settingsURL) {
