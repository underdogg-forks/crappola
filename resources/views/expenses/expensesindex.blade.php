...
<head>
    ...
    <!-- Version: 4.5.19 -->
            ...
            <title>Expenses | Invoice Ninja</title>
    ...
    <link rel="canonical" href="https://app.invoiceninja.com/expenses"/>
...
<div id="wrapper" class=' toggled-right'>
    ...
    <a href="/reports"
        ...
        Reports
            ...
            <a type="button" class="btn btn-default btn-sm pull-right" title="User Guide: Expenses &gt; List Expenses"
            href="https://invoice-ninja.readthedocs.io/en/latest/expenses.html#list-expenses" target="_blank">
...
<script type="text/javascript">
    ...
    <form accept-charset="utf-8" class="form-horizontal listForm_expense" method="POST" action="/expenses/bulk">

<div style="display:none">
	<div class="form-group"><label for="action_expense" class="control-label col-lg-4 col-sm-4">Action</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="action_expense" type="text" name="action"></div></div>
    <div class="form-group"><label for="public_id_expense" class="control-label col-lg-4 col-sm-4">public id</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="public_id_expense" type="text" name="public_id"></div></div>
...
<div class="pull-left">
						<button type='button' class='btn btn-primary invoice' onclick='submitForm_expense(&quot;invoice&quot;)'>Invoice <span class='glyphicon glyphicon-check'></span></button>
			
	<div class='btn-group'><button class='btn btn-default archive' type='button'>Archive</button><button class='btn btn-default dropdown-toggle archive' data-toggle='dropdown' type='button'><span class='caret'></span></button><ul class='dropdown-menu' role='menu' aria-labelledby='dLabel'><li><a href='javascript:submitForm_expense("archive")'>Archive Expense</a></li><li><a href='javascript:submitForm_expense("delete")'>Delete Expense</a></li></ul></div>

	&nbsp;
	<span id="statusWrapper_expense" style="display:none">
		<select class="form-control" style="width: 220px" id="statuses_expense" multiple="true">
	...
	&nbsp;
	<span class="well well-sm" id="sum_column_expense" style="display:none;padding-left:12px;padding-right:12px;"></span>
</div>

<div id="top_right_buttons" class="pull-right">
	<input id="tableFilter_expense" type="text" style="width:180px;margin-right:17px;background-color: white !important"
        class="form-control pull-left" placeholder="Filter" value=""/>

                
			<div class='btn-group'><button class='btn btn-default recurringDropdown' type='button'>Recurring</button><button class='btn btn-default dropdown-toggle recurringDropdown' data-toggle='dropdown' type='button'><span class='caret'></span></button><ul class='dropdown-menu' role='menu' aria-labelledby='dLabel'><li><a href='http://invninjv1.local/recurring_expenses/create'>New Recurring Expense</a></li></ul></div>
					<div class='btn-group'><button class='btn btn-default categoriesDropdown' type='button'>Categories</button><button class='btn btn-default dropdown-toggle categoriesDropdown' data-toggle='dropdown' type='button'><span class='caret'></span></button><ul class='dropdown-menu' role='menu' aria-labelledby='dLabel'><li><a href='http://invninjv1.local/expense_categories/create'>New Expense Category</a></li></ul></div>
			  	<script type="text/javascript">
		  	$(function() {
				$('.recurringDropdown:not(.dropdown-toggle)').click(function(event) {
					openUrlOnClick('http://invninjv1.local/recurring_expenses', event)
		  		});
				$('.categoriesDropdown:not(.dropdown-toggle)').click(function(event) {
					openUrlOnClick('http://invninjv1.local/expense_categories', event);
		  		});
			});
		</script>
	
	    	<a class='btn btn-primary' href='http://invninjv1.local/expenses/create'>Enter Expense <span class='glyphicon glyphicon-plus-sign'></span></a>
	
</div>


<table class="table table-striped data-table VIFapH0k">
    <colgroup>
                <col class="con0" />
                <col class="con1" />
                <col class="con2" />
...
<script type="text/javascript">
    ...
    function refreshDatatable_expenses() {
        window['dataTable_expenses'].api().ajax.reload();
    }

    function load_VIFapH0k() {
        window['dataTable_expenses'] = jQuery('.VIFapH0k').dataTable({
            "stateSave": true,
            "stateDuration": 0,
            ...
            "aoColumnDefs": [
                                // Disable sorting on the first column
                {
                    ...
                    'aTargets': []
                        ...
                        "sPaginationType": "bootstrap",
                        "bProcessing": false,
                        "sAjaxSource": "http:\/\/invninjv1.local\/api\/expenses",
                        "bServerSide": true,
                        "aaSorting": [[3,"desc"]],
                                    "fnDrawCallback": function(oSettings) {
                                    if (window.onDatatableReady_expenses) {
                        window.onDatatableReady_expenses();
...
<style type="text/css">
			...
			.listForm_expense table.dataTable td:nth-child(5) {
			text-align: right;
		}
	
			.listForm_expense table.dataTable td:nth-child(8) {
			text-align: center;
		}
...
<script type="text/javascript">
	...
	var submittedForm;
	function submitForm_expense(action, id) {
		...
		if (id) {
			$('#public_id_expense').val(id);
		}

		if (action == 'delete' || action == 'emailInvoice') {
	        sweetConfirm(function() {
	            $('#action_expense').val(action);
	    		$('form.listForm_expense').submit();
	        });
		} else {
			$('#action_expense').val(action);
			$('form.listForm_expense').submit();
	...
	$(function() {
	    ...
	    function filterTable_expense(val) {
	        if (val == tableFilter) {
	            return;
	        }
	        tableFilter = val;
			var oTable0 = $('.listForm_expense .data-table').dataTable();
	        oTable0.fnFilter(val);
	    }

	    $('#tableFilter_expense').on('keyup', function(){
	        if (searchTimeout) {
	            window.clearTimeout(searchTimeout);
	        }
	        searchTimeout = setTimeout(function() {
	            filterTable_expense($('#tableFilter_expense').val());
	        }, 500);
	    })

	    if ($('#tableFilter_expense').val()) {
	        filterTable_expense($('#tableFilter_expense').val());
	    }

		$('.listForm_expense .head0').click(function(event) {
			if (event.target.type !== 'checkbox') {
				$('.listForm_expense .head0 input[type=checkbox]').click();
			}
		});

		// Enable/disable bulk action buttons
	    window.onDatatableReady_expenses = function() {
	        $(':checkbox').click(function() {
	            setBulkActionsEnabled_expense();
							changeSumLabel();
	        });

	        $('.listForm_expense tbody tr').unbind('click').click(function(event) {
	    ...
	    $('.listForm_expense .archive, .invoice').prop('disabled', true);
	    $('.listForm_expense .archive:not(.dropdown-toggle)').click(function() {
	        submitForm_expense('archive');
	    });

	    $('.listForm_expense .selectAll').click(function() {
	        $(this).closest('table').find(':checkbox:not(:disabled)').prop('checked', this.checked);
	    });

	    function setBulkActionsEnabled_expense() {
	        var buttonLabel = "Archive";
	        var count = $('.listForm_expense tbody :checkbox:checked').length;
	        $('.listForm_expense button.archive, .listForm_expense button.invoice').prop('disabled', !count);
	        if (count) {
	            buttonLabel += ' (' + count + ')';
	        }
	        $('.listForm_expense button.archive').not('.dropdown-toggle').text(buttonLabel);
	    }
			...
			function changeSumLabel() {
				var dTable = $('.listForm_expense .data-table').DataTable();
					...
					if (sum) {
						$('#sum_column_expense').show().text("Total: " + sum)
					} else {
						$('#sum_column_expense').hide();
					}

				 			}

		// Setup state/status filter
		$('#statuses_expense').select2({
			placeholder: "Status",
			//allowClear: true,
			templateSelection: function(data, container) {
			  ...
			  .trigger('change')
		  .on('change', function() {
			var filter = $('#statuses_expense').val();
			if (filter) {
				filter = filter.join(',');
			} else {
				filter = '';
			}
			var url = 'http://invninjv1.local/set_entity_filter/expense' + '/' + filter;
	        $.get(url, function(data) {
	            refreshDatatable_expenses();
	        })
		}).maximizeSelect2Height();

		$('#statusWrapper_expense').show();
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a class="btn btn-primary" href="https://invoice-ninja.readthedocs.io/en/latest/expenses.html#list-expenses" target="_blank">User Guide</a>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        if ('http://invninjv1.local/expenses' != settingsURL) {
