@php use App\Ninja\Datatables\TaskDatatable; @endphp
@php use App\Ninja\Datatables\ExpenseDatatable; @endphp
@php use App\Ninja\Datatables\RecurringInvoiceDatatable; @endphp
@php use App\Ninja\Datatables\InvoiceDatatable; @endphp
@php use App\Ninja\Datatables\PaymentDatatable; @endphp
@php use App\Ninja\Datatables\CreditDatatable; @endphp
...
@section('content')
        ...
        <div class="panel-body">
            <div class="row">
                ...
                <div class="col-md-3">
                    ...
                    @if ($client->company->customLabel('client1') && $client->custom_value1)
                        {{ $client->company->present()->customLabel('client1') . ': ' }} {!! nl2br(e($client->custom_value1)) !!}
                        <br/>
                    @endif
                    @if ($client->company->customLabel('client2') && $client->custom_value2)
                        {{ $client->company->present()->customLabel('client2') . ': ' }} {!! nl2br(e($client->custom_value2)) !!}
                        <br/>
                    @endif
                    ...
                    <div class="text-muted" style="padding-top:8px">
                        ...
                        @endif
                        {{--@if ($client->company->hasReminders() && ! $client->send_reminders)
                ...
                <div class="col-md-3">
                    ...
                    @foreach ($client->contacts as $contact)
                        ...
                        @if ($client->company->customLabel('contact1') && $contact->custom_value1)
                            {{ $client->company->present()->customLabel('contact1') . ': ' . $contact->custom_value1 }}
                            <br/>
                        @endif
                        @if ($client->company->customLabel('contact2') && $contact->custom_value2)
                            {{ $client->company->present()->customLabel('contact2') . ': ' . $contact->custom_value2 }}
                            <br/>
                        @endif
                            ...
                            @if (config('services.postmark'))
                                <div style="padding-top:10px">
                                    <a href="#" class="btn btn-sm btn-primary"
                                       onclick="showEmailHistory('{{ $contact->email }}')">
    ...
    @if ($client->showMap())
        ...
        <iframe
                ...
                src="https://www.google.com/maps/embed/v1/place?key={{ env('GOOGLE_MAPS_API_KEY') }}&q={!! e("{$client->address1} {$client->address2} {$client->city} {$client->state} {$client->postal_code} " . ($client->country ? $client->country->getName() : '')) !!}"
                allowfullscreen>
        </iframe>
    ...
    <ul class="nav nav-tabs nav-justified">
        ...
        @if ($company->isModuleEnabled(ENTITY_CREDIT))
