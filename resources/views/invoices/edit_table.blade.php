<table class="table invoice-table {{ $isTasks ? 'task-table' : 'product-table' }}">
    ...
    <tr>
        ...
        @if ($company->customLabel('product1'))
            <th style="min-width:120px">{{ $company->present()->customLabel('product1') }}</th>
        @endif
        @if ($company->customLabel('product2'))
            <th style="min-width:120px">{{ $company->present()->customLabel('product2') }}</th>
        @endif
        <th style="min-width:120px">{{ $invoiceLabels[$isTasks ? 'rate' : 'unit_cost'] }}</th>
        <th style="min-width:120px;display:{{ $company->hasInvoiceField($isTasks ? 'task' : 'product', $isTasks ? 'product.hours' : 'product.quantity') ? 'table-cell' : 'none' }}">{{ $invoiceLabels[$isTasks ? 'hours' : 'quantity'] }}</th>
        <th style="min-width:120px;display:{{ $company->hasInvoiceField($isTasks ? 'task' : 'product', 'product.discount') ? 'table-cell' : 'none' }}">{{ $invoiceLabels['discount'] }}</th>
    ...
    <tr data-bind="event: { mouseover: showActions, mouseout: hideActions }" class="sortable-row">
        ...
        @if ($company->customLabel('product1'))
            <td>
                @include('partials.custom_field', [
					...
					'label' => $company->customLabel('product1'),
        ...
        @if ($company->customLabel('product2'))
            <td>
                @include('partials.custom_field', [
					...
					'label' => $company->customLabel('product2'),
        ...
        <td style="display:{{ $company->hasInvoiceField($isTasks ? 'task' : 'product', $isTasks ? 'product.hours' : 'product.quantity') ? 'table-cell' : 'none' }}">
        ...
        <td style="display:{{ $company->hasInvoiceField($isTasks ? 'task' : 'product', 'product.discount') ? 'table-cell' : 'none' }}">
