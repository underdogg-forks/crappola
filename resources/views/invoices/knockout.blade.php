<script type="text/javascript">
    ...
    function ViewModel(data) {
        ...
        self.setDueDate = function () {
            ...
            if (paymentTerms && paymentTerms != 0 && !self.invoice().due_date()) {
                ...
                dueDate = moment(dueDate).format("{{ $company->getMomentDateFormat() }}");
                $('#due_date').attr('placeholder', dueDate);
            } else {
                $('#due_date').attr('placeholder', "{{ $invoice->id ? ' ' : $company->present()->dueDatePlaceholder() }}");
            }
            @endif
        }
    ...
    function InvoiceModel(data) {
        ...
        self.formatMoney = function (amount) {
            ...
            */
            ...
            var currencyId = (self.client().currency_id() || company.currency_id) || {{ DEFAULT_CURRENCY }};
            var countryId = (self.client().country_id() || company.country_id) || {{ DEFAULT_COUNTRY }};
            var decorator = parseInt(company.show_currency_code) ? 'code' : 'symbol';
    ...
    function ItemModel(data) {
        ...
        self.qty = ko.observable({{ $company->hasInvoiceField('product', 'product.quantity') ? 0 : 1 }});
        self.discount = ko.observable();
        self.custom_value1 = ko.observable('');
        self.custom_value2 = ko.observable('');
        self.tax_name1 = ko.observable('');
        ...
        init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            $element.typeahead({
                ...
                },
                {
                    ...
                    limit: 50,
                    @if (Auth::user()->company->show_product_notes)
                    templates: {
                ...
                if (parseFloat(datum.cost)) {
                    if (!NINJA.parseFloat(model.cost()) || !model.isTask()) {
                        ...
                        var rate = false;
                        if ((company.custom_fields.invoice_text1 || '').toLowerCase() == "{{ strtolower(trans('texts.exchange_rate')) }}") {
                            rate = window.model.invoice().custom_text_value1();
                        } else if ((company.custom_fields.invoice_text2 || '').toLowerCase() == "{{ strtolower(trans('texts.exchange_rate')) }}") {
                        ...
                        if (rate) {
                            ...
                            if (client) {
                                ...
                                var accountCurrencyId = {{ $company->getCurrencyId() }};
                ...
                @endif
                        @if (Auth::user()->isPro() && $company->customLabel('product1'))
                if (datum.custom_value1) {
                    model.custom_value1(datum.custom_value1);
                }
                @endif
                        @if (Auth::user()->isPro() && $company->customLabel('product2'))
