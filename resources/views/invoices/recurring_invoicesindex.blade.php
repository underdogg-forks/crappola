...
<head>
    ...
    <!-- Version: 4.5.19 -->
            ...
            <title>Recurring Invoices | Invoice Ninja</title>
    ...
    <link rel="canonical" href="https://app.invoiceninja.com/recurring_invoices"/>
    ...
    <div class="collapse navbar-collapse" id="navbar-collapse-1">
      ...
      <ul class="nav navbar-nav hide-non-phone" style="font-weight: bold">
                                        <li><a href="/dashboard">Dashboard</a></li>
                                                      ...
                                                      <li class="active"><a href="/recurring_invoices">Recurring</a></li>
...
<div id="wrapper" class=' toggled-right'>
    ...
    <a href="/payments"
        ...
        Payments
                                                                ...
                                                                <li class="nav-recurring_invoices active">
    ...
    <a href="/reports"
        ...
        Reports
            ...
            <a type="button" class="btn btn-default btn-sm pull-right" title="User Guide: Recurring Invoices &gt; List Recurring Invoices"
            href="https://invoice-ninja.readthedocs.io/en/latest/recurring_invoices.html#list-recurring-invoices" target="_blank">
...
<script type="text/javascript">
    ...
    <form accept-charset="utf-8" class="form-horizontal listForm_recurring_invoice" method="POST" action="/recurring_invoices/bulk">

<div style="display:none">
	<div class="form-group"><label for="action_recurring_invoice" class="control-label col-lg-4 col-sm-4">Action</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="action_recurring_invoice" type="text" name="action"></div></div>
    <div class="form-group"><label for="public_id_recurring_invoice" class="control-label col-lg-4 col-sm-4">public id</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="public_id_recurring_invoice" type="text" name="public_id"></div></div>
...
<div class="pull-left">
	...
	<div class='btn-group'><button class='btn btn-default archive' type='button'>Archive</button><button class='btn btn-default dropdown-toggle archive' data-toggle='dropdown' type='button'><span class='caret'></span></button><ul class='dropdown-menu' role='menu' aria-labelledby='dLabel'><li><a href='javascript:submitForm_recurring_invoice("archive")'>Archive Recurring Invoice</a></li><li><a href='javascript:submitForm_recurring_invoice("delete")'>Delete Recurring Invoice</a></li></ul></div>

	&nbsp;
	<span id="statusWrapper_recurring_invoice" style="display:none">
		<select class="form-control" style="width: 220px" id="statuses_recurring_invoice" multiple="true">
	...
	&nbsp;
	<span class="well well-sm" id="sum_column_recurring_invoice" style="display:none;padding-left:12px;padding-right:12px;"></span>
</div>

<div id="top_right_buttons" class="pull-right">
	<input id="tableFilter_recurring_invoice" type="text" style="width:180px;margin-right:17px;background-color: white !important"
        class="form-control pull-left" placeholder="Filter" value=""/>

                
	
                    <div class='btn-group'><button class='btn btn-default recurringDropdown' type='button'>Recurring Quotes</button><button class='btn btn-default dropdown-toggle recurringDropdown' data-toggle='dropdown' type='button'><span class='caret'></span></button><ul class='dropdown-menu' role='menu' aria-labelledby='dLabel'><li><a href='http://invninjv1.local/recurring_quotes/create'>New Recurring Quote</a></li></ul></div>
        		<script type="text/javascript">
            $(function() {
                $('.recurringDropdown:not(.dropdown-toggle)').click(function(event) {
                    openUrlOnClick('http://invninjv1.local/recurring_quotes', event)
                });
            });
		</script>
	
	    	<a class='btn btn-primary' href='http://invninjv1.local/recurring_invoices/create'>New Recurring Invoice <span class='glyphicon glyphicon-plus-sign'></span></a>
	
</div>


<table class="table table-striped data-table IjdruK8S">
    <colgroup>
                <col class="con0" />
                <col class="con1" />
                <col class="con2" />
...
<script type="text/javascript">
    ...
    function refreshDatatable_recurring_invoices() {
        window['dataTable_recurring_invoices'].api().ajax.reload();
    }

    function load_IjdruK8S() {
        window['dataTable_recurring_invoices'] = jQuery('.IjdruK8S').dataTable({
            "stateSave": true,
            "stateDuration": 0,
            ...
            "aoColumnDefs": [
                                // Disable sorting on the first column
                {
                    ...
                    'aTargets': []
                        ...
                        "sPaginationType": "bootstrap",
                        "bProcessing": false,
                        "sAjaxSource": "http:\/\/invninjv1.local\/api\/recurring_invoices",
                        "bServerSide": true,
                        "aaSorting": [[1,"desc"]],
                                    "fnDrawCallback": function(oSettings) {
                                    if (window.onDatatableReady_recurring_invoices) {
                        window.onDatatableReady_recurring_invoices();
...
<style type="text/css">
			...
			.listForm_recurring_invoice table.dataTable td:nth-child(6) {
			text-align: right;
		}
	
			.listForm_recurring_invoice table.dataTable td:nth-child(8) {
			text-align: center;
		}
...
<script type="text/javascript">
	...
	var submittedForm;
	function submitForm_recurring_invoice(action, id) {
		...
		if (id) {
			$('#public_id_recurring_invoice').val(id);
		}

		if (action == 'delete' || action == 'emailInvoice') {
	        sweetConfirm(function() {
	            $('#action_recurring_invoice').val(action);
	    		$('form.listForm_recurring_invoice').submit();
	        });
		} else {
			$('#action_recurring_invoice').val(action);
			$('form.listForm_recurring_invoice').submit();
	...
	$(function() {
	    ...
	    function filterTable_recurring_invoice(val) {
	        if (val == tableFilter) {
	            return;
	        }
	        tableFilter = val;
			var oTable0 = $('.listForm_recurring_invoice .data-table').dataTable();
	        oTable0.fnFilter(val);
	    }

	    $('#tableFilter_recurring_invoice').on('keyup', function(){
	        if (searchTimeout) {
	            window.clearTimeout(searchTimeout);
	        }
	        searchTimeout = setTimeout(function() {
	            filterTable_recurring_invoice($('#tableFilter_recurring_invoice').val());
	        }, 500);
	    })

	    if ($('#tableFilter_recurring_invoice').val()) {
	        filterTable_recurring_invoice($('#tableFilter_recurring_invoice').val());
	    }

		$('.listForm_recurring_invoice .head0').click(function(event) {
			if (event.target.type !== 'checkbox') {
				$('.listForm_recurring_invoice .head0 input[type=checkbox]').click();
			}
		});

		// Enable/disable bulk action buttons
	    window.onDatatableReady_recurring_invoices = function() {
	        $(':checkbox').click(function() {
	            setBulkActionsEnabled_recurring_invoice();
							changeSumLabel();
	        });

	        $('.listForm_recurring_invoice tbody tr').unbind('click').click(function(event) {
	    ...
	    $('.listForm_recurring_invoice .archive, .invoice').prop('disabled', true);
	    $('.listForm_recurring_invoice .archive:not(.dropdown-toggle)').click(function() {
	        submitForm_recurring_invoice('archive');
	    });

	    $('.listForm_recurring_invoice .selectAll').click(function() {
	        $(this).closest('table').find(':checkbox:not(:disabled)').prop('checked', this.checked);
	    });

	    function setBulkActionsEnabled_recurring_invoice() {
	        var buttonLabel = "Archive";
	        var count = $('.listForm_recurring_invoice tbody :checkbox:checked').length;
	        $('.listForm_recurring_invoice button.archive, .listForm_recurring_invoice button.invoice').prop('disabled', !count);
	        if (count) {
	            buttonLabel += ' (' + count + ')';
	        }
	        $('.listForm_recurring_invoice button.archive').not('.dropdown-toggle').text(buttonLabel);
	    }

			function sumColumnVars(currentSum, add) {
				switch ("recurring_invoice") {
					case "task":
						if(currentSum == "") {
							currentSum = "00:00:00";
						}
			...
			function changeSumLabel() {
				var dTable = $('.listForm_recurring_invoice .data-table').DataTable();
				 				 							sumColumnNodes = dTable.column( 5 ).data().toArray();
										var sum = 0;
					...
					if (sum) {
						$('#sum_column_recurring_invoice').show().text("Total: " + sum)
					} else {
						$('#sum_column_recurring_invoice').hide();
					}

				 			}

		// Setup state/status filter
		$('#statuses_recurring_invoice').select2({
			placeholder: "Status",
			//allowClear: true,
			templateSelection: function(data, container) {
			  ...
			  .trigger('change')
		  .on('change', function() {
			var filter = $('#statuses_recurring_invoice').val();
			if (filter) {
				filter = filter.join(',');
			} else {
				filter = '';
			}
			var url = 'http://invninjv1.local/set_entity_filter/recurring_invoice' + '/' + filter;
	        $.get(url, function(data) {
	            refreshDatatable_recurring_invoices();
	        })
		}).maximizeSelect2Height();

		$('#statusWrapper_recurring_invoice').show();
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a class="btn btn-primary" href="https://invoice-ninja.readthedocs.io/en/latest/recurring_invoices.html#list-recurring-invoices" target="_blank">User Guide</a>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        if ('http://invninjv1.local/recurring_invoices' != settingsURL) {
