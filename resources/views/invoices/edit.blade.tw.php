...
@section('content')
    @if ($errors->first('invoice_items'))
        <div class="relative px-3 py-3 mb-4 border rounded text-red-darker border-red-dark bg-red-lighter">{{ trans($errors->first('invoice_items')) }}</div>
    @endif

	@if ($invoice->id)
		<ol class="flex flex-wrap list-reset pt-3 pb-3 py-4 px-4 mb-4 bg-grey-light rounded">
    ...
    <div class="flex flex-wrap" style="min-height:195px" onkeypress="formEnterClick(event)">
    	<div class="md:w-1/3 pr-4 pl-4" id="col_1">

    		@if ($invoice->id || $data)
				<div class="mb-4">
					<label for="client" class="control-label lg:w-1/3 pr-4 pl-4 sm:w-1/3 pr-4 pl-4"><b>{{ trans('texts.client') }}</b></label>
					<div class="lg:w-2/3 pr-4 pl-4 sm:w-2/3 pr-4 pl-4">
                        <h4>
			...
			<div class="mb-4" style="margin-bottom: 8px">
				<div class="lg:w-2/3 pr-4 pl-4 sm:w-2/3 pr-4 pl-4 col-lg-offset-4 col-sm-offset-4">
			...
			<div data-bind="with: client" class="invoice-contact">
				<div style="display:none" class="mb-4" data-bind="visible: contacts().length > 0, foreach: contacts">
					<div class="lg:w-2/3 pr-4 pl-4 col-lg-offset-4 col-sm-offset-4">
		...
		<div class="md:w-1/3 pr-4 pl-4" id="col_2">
			<div data-bind="visible: !is_recurring()">
				...
				<div class="mb-4 partial">
					<label for="partial" class="control-label lg:w-1/3 pr-4 pl-4 sm:w-1/3 pr-4 pl-4">{{ trans('texts.partial') }}</label>
					<div class="lg:w-2/3 pr-4 pl-4 sm:w-2/3 pr-4 pl-4 no-gutter">
						...
						<div class="lg:w-2/3 pr-4 pl-4 no-gap">
		...
		<div class="md:w-1/3 pr-4 pl-4" id="col_2">
            ...
            <span data-bind="visible: is_recurring()" style="display: none">
                ...
                <div class="mb-4" data-bind="visible: auto_bill() == {{AUTO_BILL_OPT_IN}} &amp;&amp; client_enable_auto_bill()">
                    <div class="sm:w-1/3 pr-4 pl-4 control-label">{{trans('texts.auto_bill')}}</div>
                    <div class="sm:w-2/3 pr-4 pl-4" style="padding-top:10px;padding-bottom:9px">
                        {{trans('texts.opted_in')}} - <a href="#" data-bind="click:function(){client_enable_auto_bill(false)}">({{trans('texts.disable')}})</a>
                    </div>
                </div>
                <div class="mb-4" data-bind="visible: auto_bill() == {{AUTO_BILL_OPT_OUT}} &amp;&amp; !client_enable_auto_bill()">
                    <div class="sm:w-1/3 pr-4 pl-4 control-label">{{trans('texts.auto_bill')}}</div>
                    <div class="sm:w-2/3 pr-4 pl-4" style="padding-top:10px;padding-bottom:9px">
            ...
            @if ($entityType == ENTITY_INVOICE)
            <div class="mb-4" style="margin-bottom: 8px">
                <div class="lg:w-2/3 pr-4 pl-4 sm:w-2/3 pr-4 pl-4 col-sm-offset-4 smaller" style="padding-top: 10px;">
	...
	<div class="block w-full overflow-auto scrolling-touch" style="padding-top:4px;">
		...
		<table class="pull-right subtotals-table" style="margin-right:40px; margin-top:0px;">
			...
			@if ($account->customLabel('invoice1') && $invoice->custom_taxes1)
				<tr>
					...
					<td><input name="custom_value1" class="block appearance-none w-full py-1 px-2 mb-1 text-base leading-normal bg-white text-grey-darker border border-grey rounded" data-bind="value: custom_value1, valueUpdate: 'afterkeydown'"/></td>
				</tr>
			@endif
            @if ($account->customLabel('invoice2') && $invoice->custom_taxes2)
				<tr>
					<td colspan="2">{{ $account->customLabel('invoice2') ?: trans('texts.surcharge') }}</td>
					<td><input name="custom_value2" class="block appearance-none w-full py-1 px-2 mb-1 text-base leading-normal bg-white text-grey-darker border border-grey rounded" data-bind="value: custom_value2, valueUpdate: 'afterkeydown'"/></td>
				</tr>
			@endif
		...
		<div role="tabpanel" class="pull-left" style="margin-left:40px; margin-top:30px;">
			...
			<ul class="flex flex-wrap list-reset pl-0 mb-0 border border-t-0 border-r-0 border-l-0 border-b-1 border-grey-light" role="tablist" style="border: none">
	...
	<div class="modal opacity-0" id="clientModal" tabindex="-1" role="dialog" aria-labelledby="clientModalLabel" aria-hidden="true">
	  <div class="modal-dialog" data-bind="css: {'large-dialog': $root.showMore}">
       ...
       <div class="panel panel-default">
        ...
        <div class="flex flex-wrap" data-bind="with: client" onkeypress="clientModalEnterClick(event)">
            ...
            <div style="margin-left:0px;margin-right:0px" data-bind="css: {'col-md-6': $root.showMore}">
                ...
                <div data-bind='template: { foreach: contacts,
	                ...
	                @endif
                    <div class="mb-4">
                        <div class="lg:w-2/3 pr-4 pl-4 col-lg-offset-4">
         ...
         <div class="modal-footer">
            ...
            <button type="button" class="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap py-2 px-4 rounded text-base leading-normal no-underline btn-default" data-bind="click: $root.showMoreFields, text: $root.showMore() ? '{{ trans('texts.less_fields') }}' : '{{ trans('texts.more_fields') }}'"></button>
            <button id="clientDoneButton" type="button" class="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap py-2 px-4 rounded text-base leading-normal no-underline text-blue-lightest bg-blue hover:bg-blue-light" data-bind="click: $root.clientFormComplete">{{ trans('texts.done') }}</button>
         </div>
	...
	<div class="modal opacity-0" id="recurringModal" tabindex="-1" role="dialog" aria-labelledby="recurringModalLabel" aria-hidden="true">
	  <div class="modal-dialog" style="min-width:150px">
	    <div class="modal-content">
	     ...
	     <div class="modal-footer">
	      	<button type="button" class="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap py-2 px-4 rounded text-base leading-normal no-underline text-blue-lightest bg-blue hover:bg-blue-light" data-dismiss="modal">{{ trans('texts.close') }}</button>
	     </div>
    ...
    <div class="modal opacity-0" id="recurringDueDateModal" tabindex="-1" role="dialog" aria-labelledby="recurringDueDateModalLabel" aria-hidden="true">
	  <div class="modal-dialog" style="min-width:150px">
	    <div class="modal-content">
	      <div class="modal-header">
	        <button type="button" class="absolute pin-t pin-b pin-r px-4 py-3" data-dismiss="modal" aria-hidden="true">&times;</button>
