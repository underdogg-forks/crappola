@php use App\Models\PaymentTerm; @endphp
@php use App\Models\Credit; @endphp
...
@section('content')
    ...
    <div data-bind="with: invoice">
                ...
                <div class="row" style="min-height:195px" onkeypress="formEnterClick(event)">
                    ...
                    <div class="col-md-4" id="col_2">
                        <div data-bind="visible: !is_recurring()">
                            {!! Former::text('invoice_date')->data_bind("datePicker: invoice_date, valueUpdate: 'afterkeydown'")->label($company->getLabel("{$entityType}_date"))
                            ...
                            {!! Former::text('due_at')->data_bind("datePicker: due_date, valueUpdate: 'afterkeydown'")->label($company->getLabel($invoice->getDueDateLabel()))
                                        ->placeholder($invoice->id || $invoice->isQuote() ? ' ' : $company->present()->dueDatePlaceholder())
                                        ->data_date_format(Session::get(SESSION_DATE_PICKER_FORMAT, DEFAULT_DATE_PICKER_FORMAT))->appendIcon('calendar')->addGroupClass('due_at') !!}
                        ...
                        @if (isset($frequencies))
                            <div data-bind="visible: is_recurring" style="display: none">
                                ...
                                {!! Former::select('recurring_due_date')->label($company->getLabel($invoice->getDueDateLabel()))->options($recurringDueDates)->data_bind("value: recurring_due_date")->appendIcon('question-sign')->addGroupClass('recurring_due_date') !!}
                            </div>
                        @endif
                        ...
                        @if ($company->customLabel('invoice_text1'))
                            @include('partials.custom_field', [
                                ...
                                'label' => $company->customLabel('invoice_text1'),
            ...
            {!! Former::text('invoice_number')
                ...
                <div class="form-group"
                    ...
                    <div class="col-sm-8" style="padding-top:10px;padding-bottom:9px">
                        ...
                        @endif
                        {!! Former::text('po_number')->label($company->getLabel('po_number', 'po_number_short'))->data_bind("value: po_number, valueUpdate: 'afterkeydown'") !!}
                        ...
                        @if ($company->customLabel('invoice_text2'))
                            @include('partials.custom_field', [
                                ...
                                'label' => $company->customLabel('invoice_text2'),
                                ...
                                @elseif ($invoice->id)
                                    ...
                                    @if ($invoice->is_recurring && $invoice->start_date && $invoice->is_public)
                                        @if ($sendNextDate = $invoice->getNextSendDate())
                                            {!! trans('texts.next_send_on', ['date' => '<span data-bind="tooltip: {title: \''.$invoice->getPrettySchedule().'\', html: true}">' . $company->formatDate($sendNextDate).
                ...
                <div class="table-responsive" style="padding-top:4px;">
                    ...
                    @if ($company->isModuleEnabled(ENTITY_TASK) && ($invoice->has_tasks || ! empty($tasks)))
                    ...
                    <table class="pull-right subtotals-table" style="margin-right:40px; margin-top:0px;">
                        ...
                        {{--@if ($company->customLabel('invoice1') && $invoice->custom_taxes1)
                            <tr>
                                <td colspan="2">{{ $company->customLabel('invoice1') ?: trans('texts.surcharge') }}</td>
                                <td><input name="custom_value1" class="form-control"
                                           data-bind="value: custom_value1, valueUpdate: 'afterkeydown'"/></td>
                            </tr>
                        @endif--}}
                        {{--@if ($company->customLabel('invoice2') && $invoice->custom_taxes2)
                            <tr>
                                <td colspan="2">{{ $company->customLabel('invoice2') ?: trans('texts.surcharge') }}</td>
            ...
            @if (Auth::user()->canCreateOrEdit(ENTITY_INVOICE, $invoice))
                ...
                @else
                    @if (!$invoice->is_deleted)
                        @if ( ! Auth::user()->company->realtime_preview && Auth::user()->company->live_preview)
        ...
        @include('invoices.pdf', ['company' => Auth::user()->company, 'hide_pdf' => ! Auth::user()->company->live_preview, 'realtime_preview' => Auth::user()->company->realtime_preview])
				...
				@endif
                                        ...
                                        @if (Auth::user()->hasFeature(FEATURE_INVOICE_SETTINGS))
                                            @if ($company->customLabel('client1'))
                                                @include('partials.custom_field', [
                                                    ...
                                                    'label' => $company->customLabel('client1'),
                                            ...
                                            @if ($company->customLabel('client2'))
                                                @include('partials.custom_field', [
                                                    ...
                                                    'label' => $company->customLabel('client2'),
                                            ...
                                            @if (Auth::user()->hasFeature(FEATURE_INVOICE_SETTINGS))
                                                @if ($company->customLabel('contact1'))
                                                    @include('partials.custom_field', [
                                                        ...
                                                        'label' => $company->customLabel('contact1'),
                                                        'databind' => "value: custom_value1, valueUpdate: 'afterkeydown',
                                                ...
                                                @if ($company->customLabel('contact2'))
                                                    @include('partials.custom_field', [
                                                        ...
                                                        'label' => $company->customLabel('contact2'),
                                                        'databind' => "value: custom_value2, valueUpdate: 'afterkeydown',
                    ...
                    &nbsp;
                                        ...
                                        {!! Former::select('client[currency_id]')->addOption('','')
                                                ->placeholder($company->currency ? trans('texts.currency_'.Str::slug($company->currency->name, '_')) : '')
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            @if ($company->invoice_taxes)
            @if (! empty($company->tax_name1))
            model.invoice().tax_rate1("{{ $company->tax_rate1 }}");
            model.invoice().tax_name1({!! json_encode($company->tax_name1) !!});
            @endif
            @if (! empty($company->tax_name2))
            model.invoice().tax_rate2("{{ $company->tax_rate2 }}");
            model.invoice().tax_name2({!! json_encode($company->tax_name2) !!});
            @endif
            @endif
            ...
            @endif
                ...
                NINJA.formIsChanged = true;
            model.invoice_currency_id({{ isset($expenseCurrencyId) ? $expenseCurrencyId : 0 }});
            ...
            var fields = ['invoice_date', 'due_at', 'start_date', 'end_date', 'last_sent_date'];
        ...
        function getPDFString(cb, force) {
            @if (! $invoice->id && $company->credit_number_counter > 0)
            var total = model.invoice().totals.rawTotal();
            var invoiceNumber = model.invoice().invoice_number();
            var creditNumber = "{{ $company->getNextNumber(new Credit()) }}";
        ...
        function onFormSubmit(event) {
            ...
            var expenseCurrencyId = model.invoice_currency_id();
        ...
        function setInvoiceNumber(client) {
            ...
            @if ($invoice->isQuote() && ! $company->share_counter)
                number = number.replace('{$clientCounter}', pad(client.quote_number_counter, {{ $company->invoice_number_padding }}));
            @else
                number = number.replace('{$clientCounter}', pad(client.invoice_number_counter, {{ $company->invoice_number_padding }}));
            @endif
