...
@section('head')
    ...
    @if (!empty($transactionToken) && $companyGateway->gateway_id == GATEWAY_BRAINTREE && $companyGateway->getPayPalEnabled())
    ...
    @elseif (! empty($companyGateway) && $companyGateway->getApplePayEnabled())
...
@section('content')
    ...
    <div class="container">
        ...
        <script type="text/javascript">
            ...
            $(function () {
                ...
                @if ($company->requiresAuthorization($invoice) && ! $invitation->signature_date)
    ...
    @if ($customGateway = $company->getGatewayByType(GATEWAY_TYPE_CUSTOM1))
        @include('invited.custom_gateway', ['customGateway' => $customGateway, 'number' => 1])
    @endif

    @if ($customGateway = $company->getGatewayByType(GATEWAY_TYPE_CUSTOM2))
        @include('invited.custom_gateway', ['customGateway' => $customGateway, 'number' => 2])
    @endif

    @if ($customGateway = $company->getGatewayByType(GATEWAY_TYPE_CUSTOM3))
