			...
			function payWithWepay() {
				...
				WePay.bank_account.create({
					...
					if(dataObj.bank_account_id) {
						...
						$('#wepay-error').remove();
						achLink.closest('.container').prepend($('<div id="wepay-error" style="margin-top:20px" class="relative px-3 py-3 mb-4 border rounded text-red-darker border-red-dark bg-red-lighter"></div>').text(dataObj.error_description));
					}
				});
			}
...
@section('content')
    		...
    		@elseif ($invoice->client->account->isGatewayConfigured() && floatval($invoice->balance) && !$invoice->is_recurring)
				...
				<span class="require-authorization">
	                ...
	                @elseif (count($paymentTypes) == 1)
	                    <a href='{{ $paymentURL }}' class="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap py-2 px-4 rounded text-base leading-normal no-underline text-green-lightest bg-green hover:bg-green-light py-3 px-4 text-xl leading-tight">{{ trans('texts.pay_now') }} {!! $invoice->present()->gatewayFee($gatewayTypeId) !!}</a>
	                @endif
				</span>
    		@else
    			{!! Button::normal(trans('texts.download'))->withAttributes(['onclick' => 'onDownloadClick()'])->large() !!}
	...
	@if ($account->requiresAuthorization($invoice))
		<div class="modal opacity-0" id="authorizationModal" tabindex="-1" role="dialog" aria-labelledby="authorizationModalLabel" aria-hidden="true">
			 ...
			 <div class="panel-body">
			  ...
			  <div class="modal-footer">
				 @if ($account->showAcceptTerms($invoice))
 				 ...
 				 @endif
				<button id="modalPayNowButton" type="button" class="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap py-2 px-4 rounded text-base leading-normal no-underline text-green-lightest bg-green hover:bg-green-light" onclick="onModalPayNowClick()" opacity-75="">
