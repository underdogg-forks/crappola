...
@section('body')
                        ...
                        <form id="taskForm">
							<span data-bind="event: { keypress: onFormKeyPress }">
								...
								@if (Auth::user()->hasFeature(FEATURE_INVOICE_SETTINGS))
                                    <div class="row">
										@if ($customLabel = $company->customLabel('task1'))
                                            <div style="padding-bottom: 20px; padding-right:6px;"
                                        ...
                                        @endif
                                        @if ($customLabel = $company->customLabel('task2'))
                                            <div style="padding-bottom: 20px; padding-left:6px;"
                                                 class="col-md-6 no-padding-mobile">
												@include('partials.custom_field', [
													'field' => 'custom_value2',
									...
									<tbody data-bind="foreach: selectedTask().sortedTimes">
										<tr data-bind="event: { mouseover: onMouseOver, mouseout: onMouseOut }">
											<td style="width: 25%; padding: 0 6px 10px 0">
												{!! Former::text('date')
														->placeholder($company->formatDate($company->getDateTime()))
											...
											<td style="width: 25%; padding: 0 6px 10px 6px">
												<div data-bind="css: { 'has-error': !isStartValid() }">
													{!! Former::text('start_at')
															...
															->data_bind("timepicker: startTimeOfDay, timepickerOptions: {className: 'start-time', scrollDefault: 'now', timeFormat: '" . ($company->military_time ? 'H:i:s' : 'g:i:s A') . "'}")
															->raw() !!}
												</div>
											</td>
											<td style="width: 25%; padding: 0 6px 10px 6px">
												<div data-bind="css: { 'has-error': !isEndValid() }">
													{!! Former::text('end_time')
															...
															->data_bind("timepicker: endTimeOfDay, timepickerOptions: {className: 'end-time', scrollDefault: 'now', timeFormat: '" . ($company->military_time ? 'H:i:s' : 'g:i:s A') . "'}")
															->raw() !!}
												</div>
											</td>
    ...
    <script type="text/javascript">
        ...
        var timezone = '{{ $company->getTimezone() }}';
