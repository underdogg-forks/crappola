...
@section('body')
    <nav class="relative flex flex-wrap items-center content-between py-2 px-4 navbar-default navbar-fixed-top">
        <div class="container mx-auto mx-auto">
            <div class="flex-grow items-center" style="padding-top:12px; padding-bottom:12px;">

                <!-- Navbar Buttons -->
                <ul class="flex flex-wrap list-reset pl-0 mb-0 navbar-right" style="margin-right:0px; padding-left:12px; float:right; display: none;">
                    ...
                    <button type='button' data-bind="click: onStartClick, css: startClass" class="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap py-2 px-4 rounded text-base leading-normal no-underline py-3 px-4 text-xl leading-tight">
                ...
                <div class="relative flex items-stretch w-full input-group-lg">
                    <span class="py-1 px-2 mb-1 text-base font-normal leading-normal text-grey-darkest text-center bg-grey-light border border-4 border-grey-lighter rounded" style="width:1%;" data-bind="click: onFilterClick, style: { 'background-color': (filterState() != 'all' || filterStatusId() != '') ? '#ffffaa'  : '' }" title="{{ trans('texts.filter_sort') }}"><span class="glyphicon glyphicon-filter"></span></span>
                    <input id="search" type="search" class="block appearance-none w-full py-1 px-2 mb-1 text-base leading-normal bg-white text-grey-darker border border-grey rounded search" autocomplete="off" autofocus="autofocus"
					...
					<span class="py-1 px-2 mb-1 text-base font-normal leading-normal text-grey-darkest text-center bg-grey-light border border-4 border-grey-lighter rounded" style="width:1%;" data-bind="click: onClearClick, visible: filter()" title="{{ trans('texts.clear') }}">
						<span class="glyphicon glyphicon-remove"></span>
					</span>
					<span class="py-1 px-2 mb-1 text-base font-normal leading-normal text-grey-darkest text-center bg-grey-light border border-4 border-grey-lighter rounded" style="width:1%;" data-bind="click: onRefreshClick, visible: ! filter()" title="{{ trans('texts.refresh') }}">
    ...
    <div class="container mx-auto" style="margin: 0 auto;width: 100%;">
        <div class="flex flex-wrap no-gutter">

            <!-- Task Form -->
            <div id="taskForm" class="sm:w-2/3 pr-4 pl-4 col-sm-push-4 md:w-3/5 pr-4 pl-4 col-md-push-5">
						...
						<form id="taskForm">
							<span data-bind="event: { keypress: onFormKeyPress }">
								<div class="flex flex-wrap">
									<div style="padding-bottom: 20px; padding-right:6px;" class="client-select md:w-1/2 pr-4 pl-4 no-padding-mobile">
			                            {!! Former::select('client_id')
												->addOption('', '')
												->label('client')
												->data_bind("dropdown: selectedTask().client_id, dropdownOptions: {highlighter: comboboxHighlighter}") !!}
									</div>
									<div style="padding-bottom: 20px; padding-left:6px;" class="project-select md:w-1/2 pr-4 pl-4 no-padding-mobile">
								...
								@if (Auth::user()->hasFeature(FEATURE_INVOICE_SETTINGS))
									<div class="flex flex-wrap">
										@if ($customLabel = $account->customLabel('task1'))
											<div style="padding-bottom: 20px; padding-right:6px;" class="md:w-1/2 pr-4 pl-4 no-padding-mobile">
										...
										@if ($customLabel = $account->customLabel('task2'))
											<div style="padding-bottom: 20px; padding-left:6px;" class="md:w-1/2 pr-4 pl-4 no-padding-mobile">
												@include('partials.custom_field', [
													'field' => 'custom_value2',
													'label' => $customLabel,
								...
								<table class="w-full max-w-full mb-4 bg-transparent times-table" data-bind="event: { change: selectedTask().onChange }" style="margin-bottom: 0px !important;">
            ...
            <!-- Task List -->
            <div id="taskList" class="flex flex-col pl-0 mb-0 border rounded border-grey-light sm:w-1/3 pr-4 pl-4 col-sm-pull-8 md:w-2/5 pr-4 pl-4 col-md-pull-7" style="display:none;overflow:auto;margin-bottom:0px;">
				<div id="filterPanel" style="margin-bottom:0px;padding-top:20px;padding-bottom:0px;padding-left:10px;display:none;">
					<div class="panel panel-default">
						<div class="panel-body">
							<div class="flex flex-wrap" xstyle="padding-bottom:22px;">
								@if ($statuses->count())
							...
							<div class="flex flex-wrap">
								<div class="md:w-1/2 pr-4 pl-4" style="padding-top:24px;">
	                    ...
	                    <div class="pull-right" style="text-align:right;">
	                        <div data-bind="visible: actionButtonVisible()"
	                            ...
	                            <button type="button" data-bind="css: startClass, click: onStartClick, clickBubble: false"
	                                class="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap py-2 px-4 rounded text-base leading-normal no-underline py-1 px-2 text-sm leading-tight" style="padding-left:0px; padding-right: 12px; padding-bottom: 6px; margin-top:5px;">
	                                <span data-bind="css: startIcon"></span>
	                            </button>
	                        </div>
	                    </div>
