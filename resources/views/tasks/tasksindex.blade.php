...
<head>
    ...
    <!-- Version: 4.5.19 -->
            ...
            <title>Tasks | Invoice Ninja</title>
    ...
    <link rel="canonical" href="https://app.invoiceninja.com/tasks"/>
...
<div id="wrapper" class=' toggled-right'>
    ...
    <a href="/reports"
        ...
        Reports
            ...
            <a type="button" class="btn btn-default btn-sm pull-right" title="User Guide: Tasks &gt; List Tasks"
            href="https://invoice-ninja.readthedocs.io/en/latest/tasks.html#list-tasks" target="_blank">
...
<script type="text/javascript">
    ...
    <form accept-charset="utf-8" class="form-horizontal listForm_task" method="POST" action="/tasks/bulk">

<div style="display:none">
	<div class="form-group"><label for="action_task" class="control-label col-lg-4 col-sm-4">Action</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="action_task" type="text" name="action"></div></div>
    <div class="form-group"><label for="public_id_task" class="control-label col-lg-4 col-sm-4">public id</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="public_id_task" type="text" name="public_id"></div></div>
...
<div class="pull-left">
						<button type='button' class='btn btn-primary invoice' onclick='submitForm_task(&quot;invoice&quot;)'>Invoice <span class='glyphicon glyphicon-check'></span></button>
			
	<div class='btn-group'><button class='btn btn-default archive' type='button'>Archive</button><button class='btn btn-default dropdown-toggle archive' data-toggle='dropdown' type='button'><span class='caret'></span></button><ul class='dropdown-menu' role='menu' aria-labelledby='dLabel'><li><a href='javascript:submitForm_task("archive")'>Archive Task</a></li><li><a href='javascript:submitForm_task("delete")'>Delete Task</a></li></ul></div>

	&nbsp;
	<span id="statusWrapper_task" style="display:none">
		<select class="form-control" style="width: 220px" id="statuses_task" multiple="true">
	...
	&nbsp;
	<span class="well well-sm" id="sum_column_task" style="display:none;padding-left:12px;padding-right:12px;"></span>
</div>

<div id="top_right_buttons" class="pull-right">
	<input id="tableFilter_task" type="text" style="width:180px;margin-right:17px;background-color: white !important"
        class="form-control pull-left" placeholder="Filter" value=""/>

                
			<a class='btn btn-default' href='http://invninjv1.local/tasks/kanban'>Kanban <span class='glyphicon glyphicon-th'></span></a>
		<a class='btn btn-default' href='javascript:openTimeTracker()'>Time Tracker <span class='glyphicon glyphicon-time'></span></a>
    
	    	<a class='btn btn-primary' href='http://invninjv1.local/tasks/create'>New Task <span class='glyphicon glyphicon-plus-sign'></span></a>
	
</div>


<table class="table table-striped data-table yZSnDYBE">
    <colgroup>
                <col class="con0" />
                <col class="con1" />
                <col class="con2" />
...
<script type="text/javascript">
    ...
    function refreshDatatable_tasks() {
        window['dataTable_tasks'].api().ajax.reload();
    }

    function load_yZSnDYBE() {
        window['dataTable_tasks'] = jQuery('.yZSnDYBE').dataTable({
            "stateSave": true,
            "stateDuration": 0,
            ...
            "aoColumnDefs": [
                                // Disable sorting on the first column
                {
                    ...
                    'aTargets': []
                        ...
                        "sPaginationType": "bootstrap",
                        "bProcessing": false,
                        "sAjaxSource": "http:\/\/invninjv1.local\/api\/tasks",
                        "bServerSide": true,
                        "aaSorting": [[3,"desc"]],
                                    "fnDrawCallback": function(oSettings) {
                                    if (window.onDatatableReady_tasks) {
                        window.onDatatableReady_tasks();
...
<style type="text/css">
			...
			.listForm_task table.dataTable td:nth-child(7) {
			text-align: center;
		}
...
<script type="text/javascript">
	...
	var submittedForm;
	function submitForm_task(action, id) {
	...
	$(function() {
	    ...
	    function filterTable_task(val) {
	        if (val == tableFilter) {
	            return;
	        }
	        tableFilter = val;
			var oTable0 = $('.listForm_task .data-table').dataTable();
	        oTable0.fnFilter(val);
	    }

	    $('#tableFilter_task').on('keyup', function(){
	        if (searchTimeout) {
	            window.clearTimeout(searchTimeout);
	        }
	        searchTimeout = setTimeout(function() {
	            filterTable_task($('#tableFilter_task').val());
	        }, 500);
	    })

	    if ($('#tableFilter_task').val()) {
	        filterTable_task($('#tableFilter_task').val());
	    }

		$('.listForm_task .head0').click(function(event) {
			if (event.target.type !== 'checkbox') {
				$('.listForm_task .head0 input[type=checkbox]').click();
			}
		});

		// Enable/disable bulk action buttons
	    window.onDatatableReady_tasks = function() {
	        $(':checkbox').click(function() {
	            setBulkActionsEnabled_task();
							changeSumLabel();
	        });

	        $('.listForm_task tbody tr').unbind('click').click(function(event) {
	    ...
	    $('.listForm_task .archive, .invoice').prop('disabled', true);
	    $('.listForm_task .archive:not(.dropdown-toggle)').click(function() {
	        submitForm_task('archive');
	    });

	    $('.listForm_task .selectAll').click(function() {
	        $(this).closest('table').find(':checkbox:not(:disabled)').prop('checked', this.checked);
	    });

	    function setBulkActionsEnabled_task() {
	        var buttonLabel = "Archive";
	        var count = $('.listForm_task tbody :checkbox:checked').length;
	        $('.listForm_task button.archive, .listForm_task button.invoice').prop('disabled', !count);
	        if (count) {
	            buttonLabel += ' (' + count + ')';
	        }
	        $('.listForm_task button.archive').not('.dropdown-toggle').text(buttonLabel);
	    }
			...
			function changeSumLabel() {
				var dTable = $('.listForm_task .data-table').DataTable();
				 				 							var sumColumnNodes = dTable.column( 4 ).nodes();
										var sum = 0;
					...
					if (sum) {
						$('#sum_column_task').show().text("Total: " + sum)
					} else {
						$('#sum_column_task').hide();
					}

				 			}

		// Setup state/status filter
		$('#statuses_task').select2({
			placeholder: "Status",
			//allowClear: true,
			templateSelection: function(data, container) {
			  ...
			  .trigger('change')
		  .on('change', function() {
			var filter = $('#statuses_task').val();
			if (filter) {
				filter = filter.join(',');
			} else {
				filter = '';
			}
			var url = 'http://invninjv1.local/set_entity_filter/task' + '/' + filter;
	        $.get(url, function(data) {
	            refreshDatatable_tasks();
	        })
		}).maximizeSelect2Height();

		$('#statusWrapper_task').show();
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a class="btn btn-primary" href="https://invoice-ninja.readthedocs.io/en/latest/tasks.html#list-tasks" target="_blank">User Guide</a>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        if ('http://invninjv1.local/tasks' != settingsURL) {
