...
<head>
    ...
    <!-- Version: 4.5.19 -->
            ...
            <title>Vendors | Invoice Ninja</title>
    ...
    <link rel="canonical" href="https://app.invoiceninja.com/vendors"/>
...
<div id="wrapper" class=' toggled-right'>
    ...
    <a href="/reports"
        ...
        Reports
            ...
            <a type="button" class="btn btn-default btn-sm pull-right" title="User Guide: Vendors &gt; List Vendors"
            href="https://invoice-ninja.readthedocs.io/en/latest/vendors.html#list-vendors" target="_blank">
...
<script type="text/javascript">
    ...
    <form accept-charset="utf-8" class="form-horizontal listForm_vendor" method="POST" action="/vendors/bulk">

<div style="display:none">
	<div class="form-group"><label for="action_vendor" class="control-label col-lg-4 col-sm-4">Action</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="action_vendor" type="text" name="action"></div></div>
    <div class="form-group"><label for="public_id_vendor" class="control-label col-lg-4 col-sm-4">public id</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="public_id_vendor" type="text" name="public_id"></div></div>
...
<div class="pull-left">
	...
	<div class='btn-group'><button class='btn btn-default archive' type='button'>Archive</button><button class='btn btn-default dropdown-toggle archive' data-toggle='dropdown' type='button'><span class='caret'></span></button><ul class='dropdown-menu' role='menu' aria-labelledby='dLabel'><li><a href='javascript:submitForm_vendor("archive")'>Archive Vendor</a></li><li><a href='javascript:submitForm_vendor("delete")'>Delete Vendor</a></li></ul></div>

	&nbsp;
	<span id="statusWrapper_vendor" style="display:none">
		<select class="form-control" style="width: 220px" id="statuses_vendor" multiple="true">
	...
	&nbsp;
	<span class="well well-sm" id="sum_column_vendor" style="display:none;padding-left:12px;padding-right:12px;"></span>
</div>

<div id="top_right_buttons" class="pull-right">
	<input id="tableFilter_vendor" type="text" style="width:180px;margin-right:17px;background-color: white !important"
        class="form-control pull-left" placeholder="Filter" value=""/>

                
	
	    	<a class='btn btn-primary' href='http://invninjv1.local/vendors/create'>New Vendor <span class='glyphicon glyphicon-plus-sign'></span></a>
	
</div>


<table class="table table-striped data-table 7Wjl6Jyd">
    <colgroup>
                <col class="con0" />
                <col class="con1" />
                <col class="con2" />
...
<script type="text/javascript">
    ...
    function refreshDatatable_vendors() {
        window['dataTable_vendors'].api().ajax.reload();
    }

    function load_7Wjl6Jyd() {
        window['dataTable_vendors'] = jQuery('.7Wjl6Jyd').dataTable({
            "stateSave": true,
            "stateDuration": 0,
            ...
            "aoColumnDefs": [
                                // Disable sorting on the first column
                {
                    ...
                    'aTargets': []
                        ...
                        "sPaginationType": "bootstrap",
                        "bProcessing": false,
                        "sAjaxSource": "http:\/\/invninjv1.local\/api\/vendors",
                        "bServerSide": true,
                        "aaSorting": [[4,"desc"]],
                                    "fnDrawCallback": function(oSettings) {
                                    if (window.onDatatableReady_vendors) {
                        window.onDatatableReady_vendors();
...
<script type="text/javascript">
	...
	var submittedForm;
	function submitForm_vendor(action, id) {
		...
		if (id) {
			$('#public_id_vendor').val(id);
		}

		if (action == 'delete' || action == 'emailInvoice') {
	        sweetConfirm(function() {
	            $('#action_vendor').val(action);
	    		$('form.listForm_vendor').submit();
	        });
		} else {
			$('#action_vendor').val(action);
			$('form.listForm_vendor').submit();
	...
	$(function() {
	    ...
	    function filterTable_vendor(val) {
	        if (val == tableFilter) {
	            return;
	        }
	        tableFilter = val;
			var oTable0 = $('.listForm_vendor .data-table').dataTable();
	        oTable0.fnFilter(val);
	    }

	    $('#tableFilter_vendor').on('keyup', function(){
	        if (searchTimeout) {
	            window.clearTimeout(searchTimeout);
	        }
	        searchTimeout = setTimeout(function() {
	            filterTable_vendor($('#tableFilter_vendor').val());
	        }, 500);
	    })

	    if ($('#tableFilter_vendor').val()) {
	        filterTable_vendor($('#tableFilter_vendor').val());
	    }

		$('.listForm_vendor .head0').click(function(event) {
			if (event.target.type !== 'checkbox') {
				$('.listForm_vendor .head0 input[type=checkbox]').click();
			}
		});

		// Enable/disable bulk action buttons
	    window.onDatatableReady_vendors = function() {
	        $(':checkbox').click(function() {
	            setBulkActionsEnabled_vendor();
							changeSumLabel();
	        });

	        $('.listForm_vendor tbody tr').unbind('click').click(function(event) {
	    ...
	    $('.listForm_vendor .archive, .invoice').prop('disabled', true);
	    $('.listForm_vendor .archive:not(.dropdown-toggle)').click(function() {
	        submitForm_vendor('archive');
	    });

	    $('.listForm_vendor .selectAll').click(function() {
	        $(this).closest('table').find(':checkbox:not(:disabled)').prop('checked', this.checked);
	    });

	    function setBulkActionsEnabled_vendor() {
	        var buttonLabel = "Archive";
	        var count = $('.listForm_vendor tbody :checkbox:checked').length;
	        $('.listForm_vendor button.archive, .listForm_vendor button.invoice').prop('disabled', !count);
	        if (count) {
	            buttonLabel += ' (' + count + ')';
	        }
	        $('.listForm_vendor button.archive').not('.dropdown-toggle').text(buttonLabel);
	    }
			...
			function changeSumLabel() {
				var dTable = $('.listForm_vendor .data-table').DataTable();
					...
					if (sum) {
						$('#sum_column_vendor').show().text("Total: " + sum)
					} else {
						$('#sum_column_vendor').hide();
					}

				 			}

		// Setup state/status filter
		$('#statuses_vendor').select2({
			placeholder: "Status",
			//allowClear: true,
			templateSelection: function(data, container) {
			  ...
			  .trigger('change')
		  .on('change', function() {
			var filter = $('#statuses_vendor').val();
			if (filter) {
				filter = filter.join(',');
			} else {
				filter = '';
			}
			var url = 'http://invninjv1.local/set_entity_filter/vendor' + '/' + filter;
	        $.get(url, function(data) {
	            refreshDatatable_vendors();
	        })
		}).maximizeSelect2Height();

		$('#statusWrapper_vendor').show();
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a class="btn btn-primary" href="https://invoice-ninja.readthedocs.io/en/latest/vendors.html#list-vendors" target="_blank">User Guide</a>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        if ('http://invninjv1.local/vendors' != settingsURL) {
