...
<head>
    ...
    <!-- Version: 4.5.19 -->
            ...
            <title>Tickets | Invoice Ninja</title>
    ...
    <link rel="canonical" href="https://app.invoiceninja.com/tickets"/>
...
<script type="text/javascript">
    ...
    <form accept-charset="utf-8" class="form-horizontal listForm_ticket" method="POST" action="/tickets/bulk">

<div style="display:none">
	<div class="form-group"><label for="action_ticket" class="control-label col-lg-4 col-sm-4">Action</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="action_ticket" type="text" name="action"></div></div>
    <div class="form-group"><label for="public_id_ticket" class="control-label col-lg-4 col-sm-4">public id</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="public_id_ticket" type="text" name="public_id"></div></div>
...
<div class="pull-left">
	...
	<div class='btn-group'><button class='btn btn-default archive' type='button'>Archive</button><button class='btn btn-default dropdown-toggle archive' data-toggle='dropdown' type='button'><span class='caret'></span></button><ul class='dropdown-menu' role='menu' aria-labelledby='dLabel'><li><a href='javascript:submitForm_ticket("archive")'>Archive Ticket</a></li><li><a href='javascript:submitForm_ticket("delete")'>Delete Ticket</a></li></ul></div>

	&nbsp;
	<span id="statusWrapper_ticket" style="display:none">
		<select class="form-control" style="width: 220px" id="statuses_ticket" multiple="true">
	...
	&nbsp;
	<span class="well well-sm" id="sum_column_ticket" style="display:none;padding-left:12px;padding-right:12px;"></span>
</div>

<div id="top_right_buttons" class="pull-right">
	<input id="tableFilter_ticket" type="text" style="width:180px;margin-right:17px;background-color: white !important"
        class="form-control pull-left" placeholder="Filter" value=""/>

                
	
	    	<a class='btn btn-primary' href='http://invninjv1.local/tickets/create'>New Ticket <span class='glyphicon glyphicon-plus-sign'></span></a>
	
</div>


<table class="table table-striped data-table 1vICpnGb">
    <colgroup>
                <col class="con0" />
                <col class="con1" />
                <col class="con2" />
...
<script type="text/javascript">
    ...
    function refreshDatatable_tickets() {
        window['dataTable_tickets'].api().ajax.reload();
    }

    function load_1vICpnGb() {
        window['dataTable_tickets'] = jQuery('.1vICpnGb').dataTable({
            "stateSave": true,
            "stateDuration": 0,
            ...
            "aoColumnDefs": [
                    ...
                    'aTargets': []
                        ...
                        "sPaginationType": "bootstrap",
                        "bProcessing": false,
                        "sAjaxSource": "http:\/\/invninjv1.local\/api\/tickets",
                        "bServerSide": true,
                        "aaSorting": [[1,"desc"]],
                                    "fnDrawCallback": function(oSettings) {
                                    if (window.onDatatableReady_tickets) {
                        window.onDatatableReady_tickets();
...
<style type="text/css">
			...
			.listForm_ticket table.dataTable td:nth-child(6) {
			text-align: center;
		}
...
<script type="text/javascript">
	...
	var submittedForm;
	function submitForm_ticket(action, id) {
		...
		if (id) {
			$('#public_id_ticket').val(id);
		}

		if (action == 'delete' || action == 'emailInvoice') {
	        sweetConfirm(function() {
	            $('#action_ticket').val(action);
	    		$('form.listForm_ticket').submit();
	        });
		} else {
			$('#action_ticket').val(action);
			$('form.listForm_ticket').submit();
	...
	$(function() {
	    ...
	    function filterTable_ticket(val) {
	        if (val == tableFilter) {
	            return;
	        }
	        tableFilter = val;
			var oTable0 = $('.listForm_ticket .data-table').dataTable();
	        oTable0.fnFilter(val);
	    }

	    $('#tableFilter_ticket').on('keyup', function(){
	        if (searchTimeout) {
	            window.clearTimeout(searchTimeout);
	        }
	        searchTimeout = setTimeout(function() {
	            filterTable_ticket($('#tableFilter_ticket').val());
	        }, 500);
	    })

	    if ($('#tableFilter_ticket').val()) {
	        filterTable_ticket($('#tableFilter_ticket').val());
	    }

		$('.listForm_ticket .head0').click(function(event) {
			if (event.target.type !== 'checkbox') {
				$('.listForm_ticket .head0 input[type=checkbox]').click();
			}
		});

		// Enable/disable bulk action buttons
	    window.onDatatableReady_tickets = function() {
	        $(':checkbox').click(function() {
	            setBulkActionsEnabled_ticket();
							changeSumLabel();
	        });

	        $('.listForm_ticket tbody tr').unbind('click').click(function(event) {
	    ...
	    $('.listForm_ticket .archive, .invoice').prop('disabled', true);
	    $('.listForm_ticket .archive:not(.dropdown-toggle)').click(function() {
	        submitForm_ticket('archive');
	    });

	    $('.listForm_ticket .selectAll').click(function() {
	        $(this).closest('table').find(':checkbox:not(:disabled)').prop('checked', this.checked);
	    });

	    function setBulkActionsEnabled_ticket() {
	        var buttonLabel = "Archive";
	        var count = $('.listForm_ticket tbody :checkbox:checked').length;
	        $('.listForm_ticket button.archive, .listForm_ticket button.invoice').prop('disabled', !count);
	        if (count) {
	            buttonLabel += ' (' + count + ')';
	        }
	        $('.listForm_ticket button.archive').not('.dropdown-toggle').text(buttonLabel);
	    }
			...
			function changeSumLabel() {
				var dTable = $('.listForm_ticket .data-table').DataTable();
					...
					if (sum) {
						$('#sum_column_ticket').show().text("Total: " + sum)
					} else {
						$('#sum_column_ticket').hide();
					}

				 			}

		// Setup state/status filter
		$('#statuses_ticket').select2({
			placeholder: "Status",
			//allowClear: true,
			templateSelection: function(data, container) {
			  ...
			  .trigger('change')
		  .on('change', function() {
			var filter = $('#statuses_ticket').val();
			if (filter) {
				filter = filter.join(',');
			} else {
				filter = '';
			}
			var url = 'http://invninjv1.local/set_entity_filter/ticket' + '/' + filter;
	        $.get(url, function(data) {
	            refreshDatatable_tickets();
	        })
		}).maximizeSelect2Height();

		$('#statusWrapper_ticket').show();
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        if ('http://invninjv1.local/tickets' != settingsURL) {
