...
@section('content')
    @parent
    @include('companies.nav', ['selected' => ACCOUNT_INVOICE_SETTINGS, 'advanced' => true])
    ...
    {{ Former::populateField('auto_archive_quote', intval($company->auto_archive_quote)) }}
    {{ Former::populateField('auto_email_invoice', intval($company->auto_email_invoice)) }}
    {{ Former::populateField('auto_archive_invoice', intval($company->auto_archive_invoice)) }}
    ...
    @foreach (App\Models\company::$customFields as $field)
        {{ Former::populateField("custom_fields[$field]", $company->customLabel($field)) }}
    @endforeach
    @foreach(App\Models\company::$customFieldsOptions as $field)
        {{ Former::populateField("custom_fields_options[$field]", $company->customFieldsOption($field)) }}
    @endforeach
                ...
                <div role="tabpanel" class="tab-pane" id="credit_number">
                    <div class="panel-body">
                        ...
                        {!! Former::checkbox('credit_number_enabled')
                                ...
                                ->text('enable')
                                ->value(1)
                                ->check($company->credit_number_counter > 0) !!}
                        ...
                        <div id="creditNumberDiv" style="display:none">
                            ...
                            {!! Former::inline_radios('credit_number_type')
                                    ...
                                    ])->check($company->credit_number_pattern ? 'pattern' : 'prefix') !!}
                            ...
                            {!! Former::text('credit_number_counter')
                                    ...
                                    ->help(trans('texts.credit_number_help') . ' ' .
                                        trans('texts.next_credit_number', ['number' => $company->getNextNumber(new Credit()) ?: '0001'])) !!}
                        </div>
                    </div>
                </div>
                <div role="tabpanel" class="tab-pane" id="options">
                    <div class="panel-body">
                        ...
                        {!! Former::text('reset_counter_date')
                                    ...
                                    ->data_date_start_date($company->formatDate($company->getDateTime())) !!}
                ...
                <ul class="nav nav-tabs" role="tablist" style="border: none">
                    ...
                    <li role="presentation">
                        <a href="#company_fields" aria-controls="company_fields" role="tab"
                           data-toggle="tab">{{ trans('texts.companyPlan') }}</a>
                    </li>
                </ul>
            </div>
            <div class="tab-content">
                ...
                <ul class="nav nav-tabs" role="tablist" style="border: none">
                    ...
                    @if ($company->hasFeature(FEATURE_DOCUMENTS))
                        <li role="presentation"><a href="#documents" aria-controls="documents" role="tab"
                                ...
                                @if ($count = $company->defaultDocuments->count())
                                    ({{ $count }})
                                @endif
                            </a></li>
                    @endif
                ...
                <div role="tabpanel" class="tab-pane" id="invoice_footer">
                    <div class="panel-body">
                        {!! Former::textarea('invoice_footer')
                                ...
                                ->rows(8)
                                ->raw() !!}
                        @if ($company->hasFeature(FEATURE_REMOVE_CREATED_BY) && ! $company->isTrial())
                ...
                <div class="container" style="width: 100%; padding-bottom: 0px !important">
                    <div class="panel panel-default">
                        <div class="panel-body">
                            ...
                            <ul>
                                @foreach (Invoice::$patternFields as $field)
                                    @if ($field == 'date:')
                                        <li>{$date:format}
    ...
    <script type="text/javascript">
        ...
        function setQuoteNumberEnabled() {
            ...
            $('#quote_number_counter').val(disabled ? '' : {!! json_encode($company->quote_number_counter) !!});
        }
        ...
        function onCreditNumberEnabled() {
            ...
            if (enabled) {
                ...
                $('#credit_number_counter').val({{ $company->credit_number_counter ?: 1 }});
            } else {
        ...
        var defaultDocuments = {!! $company->defaultDocuments()->get() !!};
        ...
        $(function () {
            ...
            $('#reset_counter_date').datepicker('update', '{{ Utils::fromSqlDate($company->reset_counter_date) ?: 'new Date()' }}');
