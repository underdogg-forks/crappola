    ...
    @foreach ($gateways as $gateway)
        <div id="gateway_{{ $gateway->id }}_div" class='gateway-fields' style="display: none">
			...
			@elseif ($gateway->id == GATEWAY_GOCARDLESS)
				<div class="mb-4">
		            <label class="control-label lg:w-1/3 pr-4 pl-4 sm:w-1/3 pr-4 pl-4">{{ trans('texts.webhook_url') }}</label>
		            <div class="lg:w-2/3 pr-4 pl-4 sm:w-2/3 pr-4 pl-4 help-block">
		                <input type="text"  class="block appearance-none w-full py-1 px-2 mb-1 text-base leading-normal bg-white text-grey-darker border border-grey rounded" onfocus="$(this).select()" readonly value="{{ URL::to(env('WEBHOOK_PREFIX','').'payment_hook/'.$account->account_key.'/'.GATEWAY_GOCARDLESS) }}">
            ...
            @if ($gateway->getHelp())
                <div class="mb-4">
                    <label class="control-label lg:w-1/3 pr-4 pl-4 sm:w-1/3 pr-4 pl-4"></label>
                    <div class="lg:w-2/3 pr-4 pl-4 sm:w-2/3 pr-4 pl-4 help-block">
                        {!! $gateway->getHelp() !!}
                    </div>
                </div>
        ...
        <div class="stripe-ach">
            ...
            <div class="stripe-webhook-options">
                <div class="mb-4">
                    <label class="control-label lg:w-1/3 pr-4 pl-4 sm:w-1/3 pr-4 pl-4">{{ trans('texts.webhook_url') }}</label>
                    <div class="lg:w-2/3 pr-4 pl-4 sm:w-2/3 pr-4 pl-4 help-block">
                        <input type="text"  class="block appearance-none w-full py-1 px-2 mb-1 text-base leading-normal bg-white text-grey-darker border border-grey rounded" onfocus="$(this).select()" readonly value="{{ URL::to(env('WEBHOOK_PREFIX','').'payment_hook/'.$account->account_key.'/'.GATEWAY_STRIPE) }}">
