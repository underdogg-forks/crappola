...
<head>
    ...
    <!-- Version: 4.5.19 -->
            ...
            <title>Quotes | Invoice Ninja</title>
    ...
    <link rel="canonical" href="https://app.invoiceninja.com/quotes"/>
...
<div id="wrapper" class=' toggled-right'>
    ...
    <a href="/reports"
        ...
        Reports
            ...
            <a type="button" class="btn btn-default btn-sm pull-right" title="User Guide: Quotes &gt; List Quotes"
            href="https://invoice-ninja.readthedocs.io/en/latest/quotes.html#list-quotes" target="_blank">
...
<script type="text/javascript">
    ...
    <form accept-charset="utf-8" class="form-horizontal listForm_quote" method="POST" action="/quotes/bulk">

<div style="display:none">
	<div class="form-group"><label for="action_quote" class="control-label col-lg-4 col-sm-4">Action</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="action_quote" type="text" name="action"></div></div>
    <div class="form-group"><label for="public_id_quote" class="control-label col-lg-4 col-sm-4">public id</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="public_id_quote" type="text" name="public_id"></div></div>
...
<div class="pull-left">
	...
	<div class='btn-group'><button class='btn btn-default archive' type='button'>Archive</button><button class='btn btn-default dropdown-toggle archive' data-toggle='dropdown' type='button'><span class='caret'></span></button><ul class='dropdown-menu' role='menu' aria-labelledby='dLabel'><li><a href='javascript:submitForm_quote("download")'>Download Quote</a></li><li><a href='javascript:submitForm_quote("emailInvoice")'>Email Quote</a></li><li class='divider'></li><li><a href='javascript:submitForm_quote("markSent")'>Mark Sent</a></li><li class='divider'></li><li><a href='javascript:submitForm_quote("archive")'>Archive Quote</a></li><li><a href='javascript:submitForm_quote("delete")'>Delete Quote</a></li></ul></div>

	&nbsp;
	<span id="statusWrapper_quote" style="display:none">
		<select class="form-control" style="width: 220px" id="statuses_quote" multiple="true">
	...
	&nbsp;
	<span class="well well-sm" id="sum_column_quote" style="display:none;padding-left:12px;padding-right:12px;"></span>
</div>

<div id="top_right_buttons" class="pull-right">
	<input id="tableFilter_quote" type="text" style="width:180px;margin-right:17px;background-color: white !important"
        class="form-control pull-left" placeholder="Filter" value=""/>
	    	...
	    	<a class='btn btn-primary' href='http://invninjv1.local/quotes/create'>New Quote <span class='glyphicon glyphicon-plus-sign'></span></a>
	
</div>


<table class="table table-striped data-table P1Afi7nL">
    <colgroup>
                <col class="con0" />
                <col class="con1" />
                <col class="con2" />
...
<script type="text/javascript">
    ...
    function refreshDatatable_quotes() {
        window['dataTable_quotes'].api().ajax.reload();
    }

    function load_P1Afi7nL() {
        window['dataTable_quotes'] = jQuery('.P1Afi7nL').dataTable({
            "stateSave": true,
            "stateDuration": 0,
            ...
            "aoColumnDefs": [
                                // Disable sorting on the first column
                {
                    ...
                    'aTargets': []
                        ...
                        "sPaginationType": "bootstrap",
                        "bProcessing": false,
                        "sAjaxSource": "http:\/\/invninjv1.local\/api\/quotes",
                        "bServerSide": true,
                        "aaSorting": [[3,"desc"]],
                                    "fnDrawCallback": function(oSettings) {
                                    if (window.onDatatableReady_quotes) {
                        window.onDatatableReady_quotes();
...
<style type="text/css">
			...
			.listForm_quote table.dataTable td:nth-child(5) {
			text-align: right;
		}
	
			.listForm_quote table.dataTable td:nth-child(7) {
			text-align: center;
		}
...
<script type="text/javascript">
	...
	var submittedForm;
	function submitForm_quote(action, id) {
		...
		if (id) {
			$('#public_id_quote').val(id);
		}

		if (action == 'delete' || action == 'emailInvoice') {
	        sweetConfirm(function() {
	            $('#action_quote').val(action);
	    		$('form.listForm_quote').submit();
	        });
		} else {
			$('#action_quote').val(action);
			$('form.listForm_quote').submit();
	...
	$(function() {
	    ...
	    function filterTable_quote(val) {
	        if (val == tableFilter) {
	            return;
	        }
	        tableFilter = val;
			var oTable0 = $('.listForm_quote .data-table').dataTable();
	        oTable0.fnFilter(val);
	    }

	    $('#tableFilter_quote').on('keyup', function(){
	        if (searchTimeout) {
	            window.clearTimeout(searchTimeout);
	        }
	        searchTimeout = setTimeout(function() {
	            filterTable_quote($('#tableFilter_quote').val());
	        }, 500);
	    })

	    if ($('#tableFilter_quote').val()) {
	        filterTable_quote($('#tableFilter_quote').val());
	    }

		$('.listForm_quote .head0').click(function(event) {
			if (event.target.type !== 'checkbox') {
				$('.listForm_quote .head0 input[type=checkbox]').click();
			}
		});

		// Enable/disable bulk action buttons
	    window.onDatatableReady_quotes = function() {
	        $(':checkbox').click(function() {
	            setBulkActionsEnabled_quote();
							changeSumLabel();
	        });

	        $('.listForm_quote tbody tr').unbind('click').click(function(event) {
	    ...
	    $('.listForm_quote .archive, .invoice').prop('disabled', true);
	    $('.listForm_quote .archive:not(.dropdown-toggle)').click(function() {
	        submitForm_quote('archive');
	    });

	    $('.listForm_quote .selectAll').click(function() {
	        $(this).closest('table').find(':checkbox:not(:disabled)').prop('checked', this.checked);
	    });

	    function setBulkActionsEnabled_quote() {
	        var buttonLabel = "Archive";
	        var count = $('.listForm_quote tbody :checkbox:checked').length;
	        $('.listForm_quote button.archive, .listForm_quote button.invoice').prop('disabled', !count);
	        if (count) {
	            buttonLabel += ' (' + count + ')';
	        }
	        $('.listForm_quote button.archive').not('.dropdown-toggle').text(buttonLabel);
	    }
			...
			function changeSumLabel() {
				var dTable = $('.listForm_quote .data-table').DataTable();
					...
					if (sum) {
						$('#sum_column_quote').show().text("Total: " + sum)
					} else {
						$('#sum_column_quote').hide();
					}

				 			}

		// Setup state/status filter
		$('#statuses_quote').select2({
			placeholder: "Status",
			//allowClear: true,
			templateSelection: function(data, container) {
			  ...
			  .trigger('change')
		  .on('change', function() {
			var filter = $('#statuses_quote').val();
			if (filter) {
				filter = filter.join(',');
			} else {
				filter = '';
			}
			var url = 'http://invninjv1.local/set_entity_filter/quote' + '/' + filter;
	        $.get(url, function(data) {
	            refreshDatatable_quotes();
	        })
		}).maximizeSelect2Height();

		$('#statusWrapper_quote').show();
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a class="btn btn-primary" href="https://invoice-ninja.readthedocs.io/en/latest/quotes.html#list-quotes" target="_blank">User Guide</a>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        if ('http://invninjv1.local/quotes' != settingsURL) {
