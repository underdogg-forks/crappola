...
<head>
    ...
    <!-- Version: 4.5.19 -->
            ...
            <title>Proposals | Invoice Ninja</title>
    ...
    <link rel="canonical" href="https://app.invoiceninja.com/proposals"/>
...
<div id="wrapper" class=' toggled-right'>
    ...
    <a href="/reports"
        ...
        Reports
            ...
            <a type="button" class="btn btn-default btn-sm pull-right" title="User Guide: Proposals &gt; List Proposals"
            href="https://invoice-ninja.readthedocs.io/en/latest/proposals.html#list-proposals" target="_blank">
...
<script type="text/javascript">
    ...
    <form accept-charset="utf-8" class="form-horizontal listForm_proposal" method="POST" action="/proposals/bulk">

<div style="display:none">
	<div class="form-group"><label for="action_proposal" class="control-label col-lg-4 col-sm-4">Action</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="action_proposal" type="text" name="action"></div></div>
    <div class="form-group"><label for="public_id_proposal" class="control-label col-lg-4 col-sm-4">public id</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="public_id_proposal" type="text" name="public_id"></div></div>
...
<div class="pull-left">
	...
	<div class='btn-group'><button class='btn btn-default archive' type='button'>Archive</button><button class='btn btn-default dropdown-toggle archive' data-toggle='dropdown' type='button'><span class='caret'></span></button><ul class='dropdown-menu' role='menu' aria-labelledby='dLabel'><li><a href='javascript:submitForm_proposal("archive")'>Archive Proposal</a></li><li><a href='javascript:submitForm_proposal("delete")'>Delete Proposal</a></li></ul></div>

	&nbsp;
	<span id="statusWrapper_proposal" style="display:none">
		<select class="form-control" style="width: 220px" id="statuses_proposal" multiple="true">
	...
	&nbsp;
	<span class="well well-sm" id="sum_column_proposal" style="display:none;padding-left:12px;padding-right:12px;"></span>
</div>

<div id="top_right_buttons" class="pull-right">
	<input id="tableFilter_proposal" type="text" style="width:180px;margin-right:17px;background-color: white !important"
        class="form-control pull-left" placeholder="Filter" value=""/>

                
			<div class='btn-group'><button class='btn btn-default templatesDropdown' type='button'>Templates</button><button class='btn btn-default dropdown-toggle templatesDropdown' data-toggle='dropdown' type='button'><span class='caret'></span></button><ul class='dropdown-menu' role='menu' aria-labelledby='dLabel'><li><a href='http://invninjv1.local/proposals/templates/create'>New Template</a></li></ul></div>
		  <div class='btn-group'><button class='btn btn-default snippetsDropdown' type='button'>Snippets</button><button class='btn btn-default dropdown-toggle snippetsDropdown' data-toggle='dropdown' type='button'><span class='caret'></span></button><ul class='dropdown-menu' role='menu' aria-labelledby='dLabel'><li><a href='http://invninjv1.local/proposals/snippets/create'>New Snippet</a></li></ul></div>
		<script type="text/javascript">
			$(function() {
				$('.templatesDropdown:not(.dropdown-toggle)').click(function(event) {
					openUrlOnClick('http://invninjv1.local/proposals/templates', event);
				});
				$('.snippetsDropdown:not(.dropdown-toggle)').click(function(event) {
					openUrlOnClick('http://invninjv1.local/proposals/snippets', event);
				});
			});
		</script>
	
	    	<a class='btn btn-primary' href='http://invninjv1.local/proposals/create'>New Proposal <span class='glyphicon glyphicon-plus-sign'></span></a>
	
</div>


<table class="table table-striped data-table 535bwMCf">
    <colgroup>
                <col class="con0" />
                <col class="con1" />
                <col class="con2" />
...
<script type="text/javascript">
    ...
    function refreshDatatable_proposals() {
        window['dataTable_proposals'].api().ajax.reload();
    }

    function load_535bwMCf() {
        window['dataTable_proposals'] = jQuery('.535bwMCf').dataTable({
            "stateSave": true,
            "stateDuration": 0,
            ...
            "aoColumnDefs": [
                                // Disable sorting on the first column
                {
                    ...
                    'aTargets': []
                        ...
                        "sPaginationType": "bootstrap",
                        "bProcessing": false,
                        "sAjaxSource": "http:\/\/invninjv1.local\/api\/proposals",
                        "bServerSide": true,
                        "aaSorting": [[1,"desc"]],
                                    "fnDrawCallback": function(oSettings) {
                                    if (window.onDatatableReady_proposals) {
                        window.onDatatableReady_proposals();
...
<script type="text/javascript">
	...
	var submittedForm;
	function submitForm_proposal(action, id) {
		...
		if (id) {
			$('#public_id_proposal').val(id);
		}

		if (action == 'delete' || action == 'emailInvoice') {
	        sweetConfirm(function() {
	            $('#action_proposal').val(action);
	    		$('form.listForm_proposal').submit();
	        });
		} else {
			$('#action_proposal').val(action);
			$('form.listForm_proposal').submit();
	...
	$(function() {
	    ...
	    function filterTable_proposal(val) {
	        if (val == tableFilter) {
	            return;
	        }
	        tableFilter = val;
			var oTable0 = $('.listForm_proposal .data-table').dataTable();
	        oTable0.fnFilter(val);
	    }

	    $('#tableFilter_proposal').on('keyup', function(){
	        if (searchTimeout) {
	            window.clearTimeout(searchTimeout);
	        }
	        searchTimeout = setTimeout(function() {
	            filterTable_proposal($('#tableFilter_proposal').val());
	        }, 500);
	    })

	    if ($('#tableFilter_proposal').val()) {
	        filterTable_proposal($('#tableFilter_proposal').val());
	    }

		$('.listForm_proposal .head0').click(function(event) {
			if (event.target.type !== 'checkbox') {
				$('.listForm_proposal .head0 input[type=checkbox]').click();
			}
		});

		// Enable/disable bulk action buttons
	    window.onDatatableReady_proposals = function() {
	        $(':checkbox').click(function() {
	            setBulkActionsEnabled_proposal();
							changeSumLabel();
	        });

	        $('.listForm_proposal tbody tr').unbind('click').click(function(event) {
	    ...
	    $('.listForm_proposal .archive, .invoice').prop('disabled', true);
	    $('.listForm_proposal .archive:not(.dropdown-toggle)').click(function() {
	        submitForm_proposal('archive');
	    });

	    $('.listForm_proposal .selectAll').click(function() {
	        $(this).closest('table').find(':checkbox:not(:disabled)').prop('checked', this.checked);
	    });

	    function setBulkActionsEnabled_proposal() {
	        var buttonLabel = "Archive";
	        var count = $('.listForm_proposal tbody :checkbox:checked').length;
	        $('.listForm_proposal button.archive, .listForm_proposal button.invoice').prop('disabled', !count);
	        if (count) {
	            buttonLabel += ' (' + count + ')';
	        }
	        $('.listForm_proposal button.archive').not('.dropdown-toggle').text(buttonLabel);
	    }
			...
			function changeSumLabel() {
				var dTable = $('.listForm_proposal .data-table').DataTable();
				 				 							sumColumnNodes = dTable.column( 7 ).data().toArray();
										var sum = 0;
					...
					if (sum) {
						$('#sum_column_proposal').show().text("Total: " + sum)
					} else {
						$('#sum_column_proposal').hide();
					}

				 			}

		// Setup state/status filter
		$('#statuses_proposal').select2({
			placeholder: "Status",
			//allowClear: true,
			templateSelection: function(data, container) {
			  ...
			  .trigger('change')
		  .on('change', function() {
			var filter = $('#statuses_proposal').val();
			if (filter) {
				filter = filter.join(',');
			} else {
				filter = '';
			}
			var url = 'http://invninjv1.local/set_entity_filter/proposal' + '/' + filter;
	        $.get(url, function(data) {
	            refreshDatatable_proposals();
	        })
		}).maximizeSelect2Height();

		$('#statusWrapper_proposal').show();
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a class="btn btn-primary" href="https://invoice-ninja.readthedocs.io/en/latest/proposals.html#list-proposals" target="_blank">User Guide</a>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        if ('http://invninjv1.local/proposals' != settingsURL) {
