...
<head>
    ...
    <!-- Version: 4.5.19 -->
            ...
            <title>Expense Categories | Invoice Ninja</title>
    ...
    <link rel="canonical" href="https://app.invoiceninja.com/expense_categories"/>
...
<div id="wrapper" class=' toggled-right'>
    ...
    <a href="/reports"
        ...
        Reports
            ...
            <a type="button" class="btn btn-default btn-sm pull-right" title="User Guide: Expenses &gt; Expense Categories"
            href="https://invoice-ninja.readthedocs.io/en/latest/expenses.html#expense-categories" target="_blank">
...
<script type="text/javascript">
    ...
    <form accept-charset="utf-8" class="form-horizontal listForm_expense_category" method="POST" action="/expense_categories/bulk">

<div style="display:none">
	<div class="form-group"><label for="action_expense_category" class="control-label col-lg-4 col-sm-4">Action</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="action_expense_category" type="text" name="action"></div></div>
    <div class="form-group"><label for="public_id_expense_category" class="control-label col-lg-4 col-sm-4">public id</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="public_id_expense_category" type="text" name="public_id"></div></div>
...
<div class="pull-left">
	...
	<div class='btn-group'><button class='btn btn-default archive' type='button'>Archive</button><button class='btn btn-default dropdown-toggle archive' data-toggle='dropdown' type='button'><span class='caret'></span></button><ul class='dropdown-menu' role='menu' aria-labelledby='dLabel'><li><a href='javascript:submitForm_expense_category("archive")'>Archive Category</a></li><li><a href='javascript:submitForm_expense_category("delete")'>Delete category</a></li></ul></div>

	&nbsp;
	<span id="statusWrapper_expense_category" style="display:none">
		<select class="form-control" style="width: 220px" id="statuses_expense_category" multiple="true">
	...
	&nbsp;
	<span class="well well-sm" id="sum_column_expense_category" style="display:none;padding-left:12px;padding-right:12px;"></span>
</div>

<div id="top_right_buttons" class="pull-right">
	<input id="tableFilter_expense_category" type="text" style="width:180px;margin-right:17px;background-color: white !important"
        class="form-control pull-left" placeholder="Filter" value=""/>

                
	
	    	<a class='btn btn-primary' href='http://invninjv1.local/expense_categories/create'>New Expense Category <span class='glyphicon glyphicon-plus-sign'></span></a>
	
</div>


<table class="table table-striped data-table BuvgggO0">
    <colgroup>
                <col class="con0" />
                <col class="con1" />
                <col class="con2" />
...
<script type="text/javascript">
    ...
    function refreshDatatable_expense_categories() {
        window['dataTable_expense_categories'].api().ajax.reload();
    }

    function load_BuvgggO0() {
        window['dataTable_expense_categories'] = jQuery('.BuvgggO0').dataTable({
            "stateSave": true,
            "stateDuration": 0,
            ...
            "aoColumnDefs": [
                                // Disable sorting on the first column
                {
                    ...
                    'aTargets': []
                        ...
                        "sPaginationType": "bootstrap",
                        "bProcessing": false,
                        "sAjaxSource": "http:\/\/invninjv1.local\/api\/expense_categories",
                        "bServerSide": true,
                        "aaSorting": [[1,"desc"]],
                                    "fnDrawCallback": function(oSettings) {
                                    if (window.onDatatableReady_expense_categories) {
                        window.onDatatableReady_expense_categories();
...
<script type="text/javascript">
	...
	var submittedForm;
	function submitForm_expense_category(action, id) {
		...
		if (id) {
			$('#public_id_expense_category').val(id);
		}

		if (action == 'delete' || action == 'emailInvoice') {
	        sweetConfirm(function() {
	            $('#action_expense_category').val(action);
	    		$('form.listForm_expense_category').submit();
	        });
		} else {
			$('#action_expense_category').val(action);
			$('form.listForm_expense_category').submit();
	...
	$(function() {
	    ...
	    function filterTable_expense_category(val) {
	        if (val == tableFilter) {
	            return;
	        }
	        tableFilter = val;
			var oTable0 = $('.listForm_expense_category .data-table').dataTable();
	        oTable0.fnFilter(val);
	    }

	    $('#tableFilter_expense_category').on('keyup', function(){
	        if (searchTimeout) {
	            window.clearTimeout(searchTimeout);
	        }
	        searchTimeout = setTimeout(function() {
	            filterTable_expense_category($('#tableFilter_expense_category').val());
	        }, 500);
	    })

	    if ($('#tableFilter_expense_category').val()) {
	        filterTable_expense_category($('#tableFilter_expense_category').val());
	    }

		$('.listForm_expense_category .head0').click(function(event) {
			if (event.target.type !== 'checkbox') {
				$('.listForm_expense_category .head0 input[type=checkbox]').click();
			}
		});

		// Enable/disable bulk action buttons
	    window.onDatatableReady_expense_categories = function() {
	        $(':checkbox').click(function() {
	            setBulkActionsEnabled_expense_category();
							changeSumLabel();
	        });

	        $('.listForm_expense_category tbody tr').unbind('click').click(function(event) {
	    ...
	    $('.listForm_expense_category .archive, .invoice').prop('disabled', true);
	    $('.listForm_expense_category .archive:not(.dropdown-toggle)').click(function() {
	        submitForm_expense_category('archive');
	    });

	    $('.listForm_expense_category .selectAll').click(function() {
	        $(this).closest('table').find(':checkbox:not(:disabled)').prop('checked', this.checked);
	    });

	    function setBulkActionsEnabled_expense_category() {
	        var buttonLabel = "Archive";
	        var count = $('.listForm_expense_category tbody :checkbox:checked').length;
	        $('.listForm_expense_category button.archive, .listForm_expense_category button.invoice').prop('disabled', !count);
	        if (count) {
	            buttonLabel += ' (' + count + ')';
	        }
	        $('.listForm_expense_category button.archive').not('.dropdown-toggle').text(buttonLabel);
	    }
			...
			function changeSumLabel() {
				var dTable = $('.listForm_expense_category .data-table').DataTable();
				 				 							sumColumnNodes = dTable.column( 2 ).data().toArray();
										var sum = 0;
					...
					if (sum) {
						$('#sum_column_expense_category').show().text("Total: " + sum)
					} else {
						$('#sum_column_expense_category').hide();
					}

				 			}

		// Setup state/status filter
		$('#statuses_expense_category').select2({
			placeholder: "Status",
			//allowClear: true,
			templateSelection: function(data, container) {
			  ...
			  .trigger('change')
		  .on('change', function() {
			var filter = $('#statuses_expense_category').val();
			if (filter) {
				filter = filter.join(',');
			} else {
				filter = '';
			}
			var url = 'http://invninjv1.local/set_entity_filter/expense_category' + '/' + filter;
	        $.get(url, function(data) {
	            refreshDatatable_expense_categories();
	        })
		}).maximizeSelect2Height();

		$('#statusWrapper_expense_category').show();
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a class="btn btn-primary" href="https://invoice-ninja.readthedocs.io/en/latest/expenses.html#expense-categories" target="_blank">User Guide</a>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        if ('http://invninjv1.local/expense_categories' != settingsURL) {
