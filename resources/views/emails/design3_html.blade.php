                                ...
                                <span style="font-size: 11px; color: #8f8d8e;">
                                    ...
                                    @else
                                        @if ($company->hasCustomLabel('due_at'))
                                            {{ $company->getLabel('due_at') }} {{ $company->formatDate($invoice->partial_due_date ?: $invoice->due_date) }}
                                        @else
                                            {{ utrans('texts.due_by', ['date' => $company->formatDate($invoice->partial_due_date ?: $invoice->due_date)]) }}
                                        @endif
                                    @endif
                                </span><br/>
                            @endif
                    ...
                    <td style="border-collapse: collapse; vertical-align: middle; line-height: 20px;" valign="middle">
                        <p style="margin: 0; padding: 0;">
                            @if (! isset($isRefund) || ! $isRefund)
                                ...
                                <span class="total"
                                      style="font-size: 22px; color: #FFFFFF; margin-top: 5px;display: block;">
