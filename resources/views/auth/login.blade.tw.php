...
@section('form')
    ...
    <div class="container mx-auto">
        ...
        @if (Utils::isOAuthEnabled())
            <div class="flex flex-wrap existing-accounts">
                <p>{{ trans('texts.login_or_existing') }}</p>
                @foreach (App\Services\AuthService::$providers as $provider)
                    <div class="md:w-1/4 pr-4 pl-4 sm:w-1/2 pr-4 pl-4">
                        <a href="{{ URL::to('auth/' . $provider) }}" class="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap py-2 px-4 rounded text-base leading-normal no-underline text-blue-lightest bg-blue hover:bg-blue-light py-3 px-4 text-xl leading-tight" title="{{ $provider }}"
        ...
        <div class="flex flex-wrap meta">
            ...
            @else
                <div class="md:w-3/5 pr-4 pl-4 sm:w-full pr-4 pl-4">
                    {!! link_to('/recover_password', trans('texts.recover_password')) !!}
                </div>
                <div class="md:w-2/5 pr-4 pl-4 sm:w-full pr-4 pl-4">
        ...
        @if (Utils::allowNewAccounts() && ! strstr(session('url.intended'), 'time_tracker'))
            <div class="flex flex-wrap sign-up">
                <div class="md:w-1/4 pr-4 pl-4 col-md-offset-3 sm:w-full pr-4 pl-4">
                    <h3>{{trans('texts.not_a_member_yet')}}</h3>
                    <p>{{trans('texts.login_create_an_account')}}</p>
                </div>
                <div class="md:w-1/4 pr-4 pl-4 sm:w-full pr-4 pl-4">
