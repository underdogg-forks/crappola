...
<div role="tabpanel" class="tab-pane {{ isset($active) && $active ? 'active' : '' }}" id="{{ $field }}">
    <div class="panel-body" style="padding-bottom: 0px">
        @if (isset($isReminder) && $isReminder)
            ...
            @if (floatval($fee = $company->account_email_settings->{"late_fee_quote{$number}_amount"}))
                {!! Former::populateField('late_fee_quote' . $number . '_amount', $fee) !!}
            @endif
            @if (floatval($fee = $company->account_email_settings->{"late_fee_quote{$number}_percent"}))
                ...
                <div class="row">
                    <div class="col-md-6">
                        @if ($field == 'quote_reminder4')
                            {!! Former::populateField('frequency_id_quote_reminder4', $company->account_email_settings->frequency_id_quote_reminder4) !!}
                        ...
                        @else
                            {!! Former::populateField('num_days_' . $field, $company->account_email_settings->{'num_days_' . $field}) !!}
                            {!! Former::populateField('direction_' . $field, $company->account_email_settings->{'direction_' . $field}) !!}
                            {!! Former::populateField('field_' . $field, $company->account_email_settings->{'field_' . $field}) !!}
                    ...
                    <div class="col-md-6">
                        {!! Former::populateField('enable_' . $field, $company->account_email_settings->{'enable_' . $field}) !!}
