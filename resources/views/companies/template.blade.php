...
<div role="tabpanel" class="tab-pane {{ isset($active) && $active ? 'active' : '' }}" id="{{ $field }}">
    <div class="panel-body" style="padding-bottom: 0px">
        @if (isset($isReminder) && $isReminder)
            ...
            @if (floatval($fee = $company->account_email_settings->{"late_fee{$number}_amount"}))
                {!! Former::populateField('late_fee' . $number . '_amount', $fee) !!}
            @endif
            @if (floatval($fee = $company->account_email_settings->{"late_fee{$number}_percent"}))
            ...
            <div class="well" style="padding-bottom:20px">
                <div class="row">
                    <div class="col-md-6">
                        @if ($field == 'reminder4')
                            {!! Former::populateField('frequency_id_reminder4', $company->account_email_settings->frequency_id_reminder4) !!}
