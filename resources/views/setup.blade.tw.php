...
<html lang="{{App::getLocale()}}">
  ...
  <div class="container mx-auto">
    ...
    <div class="py-8 px-4 mb-8 bg-grey-lighter rounded">
        <h2>Invoice Ninja Setup</h2>
        @if (version_compare(phpversion(), '7.0.0', '<'))
            <div class="relative px-3 py-3 mb-4 border rounded text-yellow-darker border-yellow-dark bg-yellow-lighter">Warning: The application requires PHP >= 7.0.0</div>
        @endif
        @if (!function_exists('proc_open'))
            <div class="relative px-3 py-3 mb-4 border rounded text-yellow-darker border-yellow-dark bg-yellow-lighter">Warning: <a href="http://php.net/manual/en/function.proc-open.php" target="_blank">proc_open</a> must be enabled.</div>
        @endif
        @if (!@fopen(base_path()."/.env", 'a'))
            <div class="relative px-3 py-3 mb-4 border rounded text-yellow-darker border-yellow-dark bg-yellow-lighter">Warning: Permission denied to write .env config file
