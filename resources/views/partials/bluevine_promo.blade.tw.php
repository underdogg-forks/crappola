<div class="relative px-3 py-3 mb-4 border rounded text-teal-darker border-teal-dark bg-teal-lighter" id="bluevinePromo">
    {{ trans('texts.bluevine_promo') }} &nbsp;&nbsp;
    <a href="#" onclick="showBlueVineModal()"
       class="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap py-2 px-4 rounded text-base leading-normal no-underline text-blue-lightest bg-blue hover:bg-blue-light py-1 px-2 text-sm leading-tight">{{ trans('texts.learn_more') }}</a>
    <a href="#" onclick="hideBlueVineMessage()" class="pull-right">{{ trans('texts.hide') }}</a>
</div>
<div class="modal opacity-0" id="bluevineModal" tabindex="-1" role="dialog" aria-labelledby="bluevineModalLabel"
     aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="panel-body">
                ...
                <div class="pull-right">
                  <br/>
                  <a class='inline-block align-middle text-center select-none border font-normal whitespace-no-wrap py-2 px-4 rounded text-base leading-normal no-underline text-blue-lightest bg-blue hover:bg-blue-light py-1 px-2 text-sm leading-tight' href='https://www.invoiceninja.com/blue-vine-invoice-factoring/' target="_blank">{{ trans('texts.learn_more') }}</a>
                </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap py-2 px-4 rounded text-base leading-normal no-underline text-blue-lightest bg-blue hover:bg-blue-light"
    ...
    function bluevineCreateAccount() {
        ...
        $.post(form.attr('action'), form.serialize(), function (data) {
                ...
                if (!hasOffer) {
                    ...
                    if (factoringOffer) {
                        ...
                        if (factoringOffer.is_conditional_offer) {
                            ...
                            quoteDetails.append(jQuery('<div class="flex flex-wrap">').append(
                                    jQuery('<strong class="sm:w-1/4 pr-4 pl-4">').text("{{ trans('texts.bluevine_credit_line_amount') }}"),
                                    jQuery('<div class="sm:w-1/5 pr-4 pl-4">').text(('$' + factoringOffer.credit_line_amount).replace(/(\d)(?=(\d{3})+$)/g, '$1,'))// Add commas to number
                            ));
                            ...
                            quoteDetails.append(jQuery('<div class="flex flex-wrap">').append(
                                    jQuery('<strong class="sm:w-1/4 pr-4 pl-4">').text("{{ trans('texts.bluevine_advance_rate') }}"),
                                    jQuery('<div class="sm:w-1/5 pr-4 pl-4">').text(advanceRate + '%')
                            ));

                            quoteDetails.append(jQuery('<div class="flex flex-wrap">').append(
                                    jQuery('<strong class="sm:w-1/4 pr-4 pl-4">').text("{{ trans('texts.bluevine_weekly_discount_rate') }}"),
                                    jQuery('<div class="sm:w-1/5 pr-4 pl-4">').text(factoringOffer.weekly_discount_rate + '%')
                            ));

                            quoteDetails.append(jQuery('<div class="flex flex-wrap">').append(
                                    jQuery('<strong class="sm:w-1/4 pr-4 pl-4">').text("{{ trans('texts.bluevine_minimum_fee_rate') }}"),
                                    jQuery('<div class="sm:w-1/5 pr-4 pl-4">').text(factoringOffer.minimum_fee_rate + '%')
                            ));
                        } else {
                    ...
                    if (locOffer) {
                        ...
                        if (locOffer.is_conditional_offer) {
                            ...
                            quoteDetails.append(jQuery('<div class="flex flex-wrap">').append(
                                    jQuery('<strong class="sm:w-1/4 pr-4 pl-4">').text("{{ trans('texts.bluevine_credit_line_amount') }}"),
                                    jQuery('<div class="sm:w-1/5 pr-4 pl-4">').text(('$' + locOffer.credit_line_amount).replace(/(\d)(?=(\d{3})+$)/g, '$1,'))// Add commas to number
                            ));

                            quoteDetails.append(jQuery('<div class="flex flex-wrap">').append(
                                    jQuery('<strong class="sm:w-1/4 pr-4 pl-4">').text("{{ trans('texts.bluevine_interest_rate') }}"),
                                    jQuery('<div class="sm:w-1/5 pr-4 pl-4">').text(locOffer.interest_rate + '%')
                            ));

                            quoteDetails.append(jQuery('<div class="flex flex-wrap">').append(
                                    jQuery('<strong class="sm:w-1/4 pr-4 pl-4">').text("{{ trans('texts.bluevine_weekly_draw_rate') }}"),
                                    jQuery('<div class="sm:w-1/5 pr-4 pl-4">').text(locOffer.weekly_draw_rate + '%')
                            ));
                        } else {
                            quoteDetails.append(jQuery('<p>').text("{{trans('texts.bluevine_no_conditional_offer')}}"));
                        }
                    ...
                    /*<div class="flex flex-wrap"><strong class="sm:w-1/3 pr-4 pl-4">Credit Line Amount</strong>  <div class="sm:w-1/5 pr-4 pl-4">$60,000</div>
                     </div>
                     <div class="flex flex-wrap">
                     <strong class="sm:w-1/3 pr-4 pl-4">Advance Rate</strong>  <div class="sm:w-1/3 pr-4 pl-4">90%</div>
                     </div>
                     <div class="flex flex-wrap">
                     <strong class="sm:w-1/3 pr-4 pl-4">Weekly Discount Rate</strong>  <div class="sm:w-1/5 pr-4 pl-4">0.8%</div>
                     </div>

                     <div class="flex flex-wrap">
                     <strong class="sm:w-1/3 pr-4 pl-4">Minimum Rate</strong>  <div class="sm:w-1/5 pr-4 pl-4">1.5%</div>
                     </div>*/
                }

                $('#bluevineModal .btn-primary').replaceWith(
                        jQuery('<a class="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap py-2 px-4 rounded text-base leading-normal no-underline text-blue-lightest bg-blue hover:bg-blue-light">').attr('href', redirectUrl).text("{{ trans('texts.bluevine_continue') }}")
                )
            } else {
                $('#bluevineModal .panel-body').append(
                        jQuery('<div class="relative px-3 py-3 mb-4 border rounded text-red-darker border-red-dark bg-red-lighter">').text(data.message ? data.message : "{{ trans('texts.bluevine_unexpected_error') }}")
                );
            }
            ...
            $('#bluevineModal .btn-primary').removeAttr('disabled');
                ...
                function () {
                    $('#bluevineModal .panel-body').append(
                            jQuery('<div class="relative px-3 py-3 mb-4 border rounded text-red-darker border-red-dark bg-red-lighter">').text("{{ trans('texts.bluevine_unexpected_error') }}")
                    );
                    $('#bluevineModal .btn-primary').removeAttr('disabled');
                }
        );
