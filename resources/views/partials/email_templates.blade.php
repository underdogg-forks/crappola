...
@php use App\Models\GatewayType; @endphp
<script type="text/javascript">
    ...
    function renderEmailTemplate(str, invoice, entityType) {
        ...
        var passwordHtml = {!! $company->isPro() && $company->enable_portal_password && $company->send_portal_password ? json_encode('<br/>' . trans('texts.password') . ': XXXXXXXXX<br/>') : json_encode('') !!};
        ...
        var keys = {
            ...
            'emailSignature': {!! json_encode($company->getEmailFooter()) !!},
            'company': "{{ $company->getDisplayName() }}",
            'dueDate': invoice ? invoice.partial_due_date || invoice.due_date : "{{ $company->formatDate($company->getDateTime()) }}",
            'invoiceDate': invoice ? invoice.invoice_date : "{{ $company->formatDate($company->getDateTime()) }}",
            ...
            'amount': invoice ? formatMoneyInvoice(parseFloat(invoice.partial) || parseFloat(invoice.balance_amount), invoice) : formatMoneyAccount(100, company),
            'balance': invoice ? formatMoneyInvoice(parseFloat(invoice.balance), invoice) : formatMoneyAccount(100, company),
            'total': invoice ? formatMoneyInvoice(parseFloat(invoice.amount), invoice) : formatMoneyAccount(100, company),
            'partial': invoice ? formatMoneyInvoice(parseFloat(invoice.partial), invoice) : formatMoneyAccount(10, company),
            ...
            'viewLink': '{{ link_to('#', auth()->user()->company->getBaseUrl() . '/...') }}$password',
            'viewButton': viewButton,
            'paymentLink': '{{ link_to('#', auth()->user()->company->getBaseUrl() . '/...') }}$password',
            'paymentButton': {!! json_encode(Form::flatButton('pay_now', '#36c157')) !!} + '$password',
            'approveLink': '{{ link_to('#', auth()->user()->company->getBaseUrl() . '/...') }}$password',
            'approveButton': {!! json_encode(Form::flatButton('approve', '#36c157')) !!} + '$password',
            'autoBill': '{{ trans('texts.auto_bill_notification_placeholder') }}',
            'portalLink': "{{ auth()->user()->company->getBaseUrl() . '/...' }}",
        ...
        @foreach (Gateway::$gatewayTypes as $type)
                ...
                {!! "keys['" . Utils::toCamelCase(GatewayType::getAliasFromId($type)) . "Link'] = '" . auth()->user()->company->getBaseUrl() . "/...';" !!}
                {!! "keys['" . Utils::toCamelCase(GatewayType::getAliasFromId($type)) . "Button'] = '" . Form::flatButton('pay_now', '#36c157') . "';" !!}
                @endif
                @endforeach

            var
        includesPasswordPlaceholder = str.indexOf('$password') != -1;
                    ...
                    <div class="panel-body">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <ul>
                                    ...
                                    @foreach (Gateway::$gatewayTypes as $type)
                                        @if ($company->getGatewayByType($type))
                                            @if ($type != GATEWAY_TYPE_TOKEN)
                                                <li>
                                                    ${{ Utils::toCamelCase(GatewayType::getAliasFromId($type)) }}
                                                    Link
                                                </li>
                                                <li>
                                                    ${{ Utils::toCamelCase(GatewayType::getAliasFromId($type)) }}
                                ...
                                @if ($company->customLabel('client1') || $company->customLabel('contact1') || $company->customLabel('invoice_text1'))
