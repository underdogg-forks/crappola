...
<head>
    ...
    <!-- Version: 4.5.19 -->
            ...
            <title>Credits | Invoice Ninja</title>
    ...
    <link rel="canonical" href="https://app.invoiceninja.com/credits"/>
...
<div id="wrapper" class=' toggled-right'>
    ...
    <a href="/reports"
        ...
        Reports
            ...
            <a type="button" class="btn btn-default btn-sm pull-right" title="User Guide: Credits &gt; List Credits"
            href="https://invoice-ninja.readthedocs.io/en/latest/credits.html#list-credits" target="_blank">
...
<script type="text/javascript">
    ...
    <form accept-charset="utf-8" class="form-horizontal listForm_credit" method="POST" action="/credits/bulk">

<div style="display:none">
	<div class="form-group"><label for="action_credit" class="control-label col-lg-4 col-sm-4">Action</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="action_credit" type="text" name="action"></div></div>
    <div class="form-group"><label for="public_id_credit" class="control-label col-lg-4 col-sm-4">public id</label><div class="col-lg-8 col-sm-8"><input class="form-control" id="public_id_credit" type="text" name="public_id"></div></div>
...
<div class="pull-left">
	...
	<div class='btn-group'><button class='btn btn-default archive' type='button'>Archive</button><button class='btn btn-default dropdown-toggle archive' data-toggle='dropdown' type='button'><span class='caret'></span></button><ul class='dropdown-menu' role='menu' aria-labelledby='dLabel'><li><a href='javascript:submitForm_credit("archive")'>Archive Credit</a></li><li><a href='javascript:submitForm_credit("delete")'>Delete Credit</a></li></ul></div>

	&nbsp;
	<span id="statusWrapper_credit" style="display:none">
		<select class="form-control" style="width: 220px" id="statuses_credit" multiple="true">
	...
	&nbsp;
	<span class="well well-sm" id="sum_column_credit" style="display:none;padding-left:12px;padding-right:12px;"></span>
</div>

<div id="top_right_buttons" class="pull-right">
	<input id="tableFilter_credit" type="text" style="width:180px;margin-right:17px;background-color: white !important"
        class="form-control pull-left" placeholder="Filter" value=""/>

                
	
	    	<a class='btn btn-primary' href='http://invninjv1.local/credits/create'>Enter Credit <span class='glyphicon glyphicon-plus-sign'></span></a>
	
</div>


<table class="table table-striped data-table 1Xd8qVC7">
    <colgroup>
                <col class="con0" />
                <col class="con1" />
                <col class="con2" />
...
<script type="text/javascript">
    ...
    function refreshDatatable_credits() {
        window['dataTable_credits'].api().ajax.reload();
    }

    function load_1Xd8qVC7() {
        window['dataTable_credits'] = jQuery('.1Xd8qVC7').dataTable({
            "stateSave": true,
            "stateDuration": 0,
            ...
            "aoColumnDefs": [
                                // Disable sorting on the first column
                {
                    ...
                    'aTargets': []
                        ...
                        "sPaginationType": "bootstrap",
                        "bProcessing": false,
                        "sAjaxSource": "http:\/\/invninjv1.local\/api\/credits",
                        "bServerSide": true,
                        "aaSorting": [[4,"desc"]],
                                    "fnDrawCallback": function(oSettings) {
                                    if (window.onDatatableReady_credits) {
                        window.onDatatableReady_credits();
...
<style type="text/css">
			...
			.listForm_credit table.dataTable td:nth-child(3) {
			text-align: right;
		}
			.listForm_credit table.dataTable td:nth-child(4) {
			text-align: right;
		}
...
<script type="text/javascript">
	...
	var submittedForm;
	function submitForm_credit(action, id) {
		...
		if (id) {
			$('#public_id_credit').val(id);
		}

		if (action == 'delete' || action == 'emailInvoice') {
	        sweetConfirm(function() {
	            $('#action_credit').val(action);
	    		$('form.listForm_credit').submit();
	        });
		} else {
			$('#action_credit').val(action);
			$('form.listForm_credit').submit();
	...
	$(function() {
	    ...
	    function filterTable_credit(val) {
	        if (val == tableFilter) {
	            return;
	        }
	        tableFilter = val;
			var oTable0 = $('.listForm_credit .data-table').dataTable();
	        oTable0.fnFilter(val);
	    }

	    $('#tableFilter_credit').on('keyup', function(){
	        if (searchTimeout) {
	            window.clearTimeout(searchTimeout);
	        }
	        searchTimeout = setTimeout(function() {
	            filterTable_credit($('#tableFilter_credit').val());
	        }, 500);
	    })

	    if ($('#tableFilter_credit').val()) {
	        filterTable_credit($('#tableFilter_credit').val());
	    }

		$('.listForm_credit .head0').click(function(event) {
			if (event.target.type !== 'checkbox') {
				$('.listForm_credit .head0 input[type=checkbox]').click();
			}
		});

		// Enable/disable bulk action buttons
	    window.onDatatableReady_credits = function() {
	        $(':checkbox').click(function() {
	            setBulkActionsEnabled_credit();
							changeSumLabel();
	        });

	        $('.listForm_credit tbody tr').unbind('click').click(function(event) {
	    ...
	    $('.listForm_credit .archive, .invoice').prop('disabled', true);
	    $('.listForm_credit .archive:not(.dropdown-toggle)').click(function() {
	        submitForm_credit('archive');
	    });

	    $('.listForm_credit .selectAll').click(function() {
	        $(this).closest('table').find(':checkbox:not(:disabled)').prop('checked', this.checked);
	    });

	    function setBulkActionsEnabled_credit() {
	        var buttonLabel = "Archive";
	        var count = $('.listForm_credit tbody :checkbox:checked').length;
	        $('.listForm_credit button.archive, .listForm_credit button.invoice').prop('disabled', !count);
	        if (count) {
	            buttonLabel += ' (' + count + ')';
	        }
	        $('.listForm_credit button.archive').not('.dropdown-toggle').text(buttonLabel);
	    }
			...
			function changeSumLabel() {
				var dTable = $('.listForm_credit .data-table').DataTable();
					...
					if (sum) {
						$('#sum_column_credit').show().text("Total: " + sum)
					} else {
						$('#sum_column_credit').hide();
					}

				 			}

		// Setup state/status filter
		$('#statuses_credit').select2({
			placeholder: "Status",
			//allowClear: true,
			templateSelection: function(data, container) {
			  ...
			  .trigger('change')
		  .on('change', function() {
			var filter = $('#statuses_credit').val();
			if (filter) {
				filter = filter.join(',');
			} else {
				filter = '';
			}
			var url = 'http://invninjv1.local/set_entity_filter/credit' + '/' + filter;
	        $.get(url, function(data) {
	            refreshDatatable_credits();
	        })
		}).maximizeSelect2Height();

		$('#statusWrapper_credit').show();
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a class="btn btn-primary" href="https://invoice-ninja.readthedocs.io/en/latest/credits.html#list-credits" target="_blank">User Guide</a>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        if ('http://invninjv1.local/credits' != settingsURL) {
