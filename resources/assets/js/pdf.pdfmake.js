        ...
        if (invoice.features.customize_invoice_design) {
            if (key === 'header') {
                return function (page, pages) {
                    if (page === 1 || invoice.company.all_pages_header == '1') {
                ...
                return function (page, pages) {
                    if (page === pages || invoice.company.all_pages_footer == '1') {
                        if (invoice.features.remove_created_by) {
                            return NINJA.updatePageCount(JSON.parse(JSON.stringify(val)), page, pages);
                        } else {
                            return val;
...
NINJA.decodeJavascript = function (invoice, javascript) {
    var corporation = invoice.company;
...
NINJA.statementInvoices = function (invoice) {
    ...
    for (var i = 0; i < invoice.invoice_items.length; i++) {
        ...
        grid.push([
            ...
            {
                text: item.custom_value1 && item.custom_value1 != '0000-00-00' ? moment(item.custom_value1).format(invoice.company.date_format ? invoice.company.date_format.format_moment : 'MMM D, YYYY') : ' ',
                style: ['invoiceDate', rowStyle]
            },
            {
                text: item.custom_value2 && item.custom_value2 != '0000-00-00' ? moment(item.custom_value2).format(invoice.company.date_format ? invoice.company.date_format.format_moment : 'MMM D, YYYY') : ' ',
...
NINJA.signatureDate = function (invoice) {
    var invitation = NINJA.getSignatureInvitation(invoice);
    return invitation ? NINJA.formatDateTime(invitation.signature_date, invoice.company) : '';
}
...
NINJA.getSignatureInvitation = function (invoice) {
    ...
    if (!parseInt(invoice.company.signature_on_pdf)) {
        return false;
    }
...
NINJA.productFields = function (invoice, isTasks) {
    ...
    if (invoice.company.hide_quantity == '1' && !isTasks) {
        fields.splice(5, 1);
    }
...
NINJA.subtotals = function (invoice, hideBalance) {
    ...
    if (!invoice.is_quote && invoice.balance_amount >= 0 && (invoice.company.hide_paid_to_date != '1' || paid)) {
        data.push([{
...
NINJA.renderField = function (invoice, field, twoColumn) {
    ...
    if (field == 'client.client_name') {
        ...
        if (invoice.company.custom_fields.account1 && invoice.company.custom_value1) {
            label = invoice.company.custom_fields.account1;
            value = invoice.company.custom_value1;
        }
    } else if (field == 'corporation.custom_value2') {
        if (invoice.company.custom_fields.account2 && invoice.company.custom_value2) {
            label = invoice.company.custom_fields.account2;
            value = invoice.company.custom_value2;
        }
        ...
        if (invoice.custom_text_value1 && corporation.custom_fields.invoice_text1) {
            label = NINJA.getCustomLabel(invoice.company.custom_fields.invoice_text1);
        ...
        if (invoice.custom_text_value2 && corporation.custom_fields.invoice_text2) {
            label = NINJA.getCustomLabel(invoice.company.custom_fields.invoice_text2);
