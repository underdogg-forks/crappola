...
NINJA.decodeJavascript = function (invoice, javascript) {
    ...
    if (matches) {
        for (var i = 0; i < matches.length; i++) {
            ...
            } else if (match == '$company.phone') {
                field = 'company.work_phone';
...
NINJA.formatDateTime = function (date, company) {
    var format = company.datetime_format ? company.datetime_format.format_moment : 'LLL';
    var timezone = company.timezone ? company.timezone.name : 'US/Eastern';
...
NINJA.productFields = function (invoice, isTasks) {
    var company = invoice.company;
    var allFields = JSON.parse(company.invoice_fields);
...
NINJA.invoiceLines = function (invoice, isSecondTable) {
    ...
    for (var i = 0; i < fields.length; i++) {
        ...
        if (field == 'custom_value1') {
            if (invoice.has_custom_item_value1) {
                value = NINJA.getCustomLabel(company.custom_fields.product1);
            } else {
                continue;
            }
        } else if (field == 'custom_value2') {
            if (invoice.has_custom_item_value2) {
                value = NINJA.getCustomLabel(company.custom_fields.product2);
            } else {
                continue;
            }
...
NINJA.subtotals = function (invoice, hideBalance) {
    ...
    var customValue1Label = company.custom_fields.invoice1 || invoiceLabels.surcharge;

    var customValue2 = NINJA.parseFloat(invoice.custom_value2);
    var customValue2Label = company.custom_fields.invoice2 || invoiceLabels.surcharge;
...
NINJA.renderField = function (invoice, field, twoColumn) {
    ...
    if (field == 'client.client_name') {
        ...
        if (company.custom_fields.client1 && client.custom_value1) {
            label = NINJA.getCustomLabel(company.custom_fields.client1);
            value = client.custom_value1;
        }
    } else if (field == 'client.custom_value2') {
        if (company.custom_fields.client2 && client.custom_value2) {
            label = NINJA.getCustomLabel(company.custom_fields.client2);
            value = client.custom_value2;
        }
    } else if (field == 'contact.custom_value1') {
        if (company.custom_fields.contact1 && contact.custom_value1) {
            label = NINJA.getCustomLabel(company.custom_fields.contact1);
            value = contact.custom_value1;
        }
    } else if (field == 'contact.custom_value2') {
        if (company.custom_fields.contact2 && contact.custom_value2) {
            label = NINJA.getCustomLabel(company.custom_fields.contact2);
        ...
        value = company.country ? company.country.name : false;
        ...
        if (invoice.custom_text_value1 && company.custom_fields.invoice_text1) {
        ...
        if (invoice.custom_text_value2 && company.custom_fields.invoice_text2) {
    ...
    if (value) {
        ...
        var style = snakeToCamel(shortField == 'company_name' ? 'company_name' : shortField); // backwards compatibility
        if (twoColumn) {
