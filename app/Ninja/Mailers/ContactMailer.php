...
class ContactMailer extends Mailer
{
    ...
    )
    {
        ...
        } elseif (!$user->confirmed || $this->isThrottled($company)) {
        ...
        if (!$proposal) {
            ...
            if (empty($invitation->contact->password) && $company->isClientPortalPasswordEnabled() && $company->send_portal_password) {
        ...
        $fromEmail = $company->getReplyToEmail() ?: $user->email;
    ...
    private function isThrottled($company)
    {
        if (Utils::isSelfHost()) {
            return false;
        }
        ...
        $day = 60 * 60 * 24;
        $day_limit = $company->getDailyEmailLimit();
        ...
        if ($new_day_throttle > $day) {
            ...
            if ($errorEmail && !Cache::get("throttle_notified:{$key}")) {
                Mail::raw('company Throttle: ' . $company->account_key, function ($message) use ($errorEmail, $company): void {
    ...
    public function sendPaymentConfirmation(Payment $payment, $refunded = 0): void
    {
        ...
        if ($refunded > 0) {
            ...
            $emailTemplate = trans('texts.refund_body', [
                'amount' => $company->formatMoney($refunded, $client),
        ...
        if ($user->email && $contact->email) {
            $this->sendTo($contact->email, $fromEmail, $companyName, $subject, $view, $data);
        }
     ...
     * @param $productId
     */
    public function sendLicensePaymentConfirmation($name, $email, $amount, $license, $productId): void
    {
    ...
    public function sendPasswordReset($contact, $token): void
    {
        if (!$contact->email) {
            return;
        }
