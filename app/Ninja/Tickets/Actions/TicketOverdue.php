...
class TicketOverdue extends BaseTicketAction
{
    ...
    public function fire(Ticket $ticket): void
    {
        ...
        $companyTicketSettings = $company->account_ticket_settings;
        ...
        if ($companyTicketSettings->alert_ticket_overdue_agent_id > 0 && $ticket->agent_id > 0) {
            $toEmail = $ticket->agent->email;
            $fromEmail = $this->buildFromLocalAddress($company, $companyTicketSettings);
            ...
            $data = [
                ...
                'body' => parent::buildTicketBodyResponse($ticket, $companyTicketSettings, $companyTicketSettings->alert_ticket_overdue_agent_id),
                'company' => $company,
    ...
    /**
     * @param $company
     * @param $companyTicketSettings
     *
     * @return string
     */
    public function buildFromLocalAddress($company, $companyTicketSettings): string
    {
        $fromName = $companyTicketSettings->support_email_local_part;

        if (Utils::isNinjaProd()) {
            $domainName = Domain::getSupportDomainFromId($company->domain_id);
        } else {
