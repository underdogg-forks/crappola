...
class InboundTicketService
{
    ...
    public function process()
    {
        ...
        if ($ticket_hash = $this->inboundTicketFactory->mailboxHash()) {
            ...
            $companyTicketSettings = AccountTicketSettings::where('support_email_local_part', '=', $localPart)->first();

            // Test if we have found a matching company
            if (! $companyTicketSettings) {
                return;
            }

            $ticket = Ticket::whereCompanyPlanId($companyTicketSettings->company_id)
    ...
    private function processTicket(Ticket $ticket, array $data, $user)
    {
        ...
        foreach ($this->inboundTicketFactory->attachments() as $attachment) {
            ...
            $documentRepo->inboundUpload($doc, $user->company);
        }
    ...
    private function checkSupportEmailAttempt()
    {
        ...
        $parts = explode('@', $to);
        $companyTicketSettings = AccountTicketSettings::where('support_email_local_part', $parts[0])->first();

        /**
         * harvest the contact using the company and contact email address.
         */
        $from = $this->inboundTicketFactory->fromEmail();

        if ($companyTicketSettings) {
            $contacts = Contact::whereCompanyPlanId($companyTicketSettings->company_id)
                ->whereEmail($from)->get();

            if (count($contacts) == 1 && ($companyTicketSettings->allow_inbound_email_tickets_external == true)) {
                // Most use cases will hit the following. A single contact sending in a support ticket request.

                return $this->createTicket($companyTicketSettings->ticket_master, $contacts[0]);
            } elseif (count($contacts) > 1 && ($companyTicketSettings->allow_inbound_email_tickets_external == true)) {
                /*
                 * Handle an edge case where one email address is registered across two different companies.
                 ...
                 */
                return $this->createClientlessTicket($companyTicketSettings->ticket_master, $from, $companyTicketSettings->company);
     ...
     * @return $ticket
     */
    private function createClientlessTicket($user, $contactEmail, $company)
    {
        $data = [
