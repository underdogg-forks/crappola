...
class StripePaymentDriver extends BasePaymentDriver
{
    ...
    public function gatewayTypes()
    {
        ...
        if ($gateway = $this->accountGateway) {
            ...
            if ($achEnabled && $sofortEnabled) {
                if ($this->invitation) {
                    $country = ($this->client() && $this->client()->country) ? $this->client()->country->iso_3166_3 : ($this->company()->country ? $this->company()->country->iso_3166_3 : false);
    ...
    public function verifyBankAccount($client, $publicId, $amount1, $amount2)
    {
        ...
        if (is_string($result) && $result != 'This bank company has already been verified.') {
            return $result;
        }
    ...
    public function createSource()
    {
        ...
        $country = $this->client()->country ? $this->client()->country->iso_3166_2 : ($this->company()->country ? $this->company()->country->iso_3166_2 : '');
        $extra = '';
    ...
    public function handleWebHook($input)
    {
        ...
        if ($eventType == 'charge.failed' || $eventType == 'charge.succeeded' || $eventType == 'charge.refunded') {
            ...
            $this->invitation = Invitation::scope(false, $companyId)->where('transaction_reference', '=', $sourceRef)->first();
            if (! $this->invitation) {
                return false;
            }
