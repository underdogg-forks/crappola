...
class BasePaymentDriver
{
    ...
    {
        $this->accountGateway = $companyGateway;
    ...
    protected function meetsGatewayTypeLimits($gatewayTypeId)
    {
        ...
        $companyGatewaySettings = AccountGatewaySettings::scope(false, $this->invitation->company_id)
    ...
    private function paymentDetailsFromInput($input)
    {
        ...
        $data = [
            'companyPlan' => $client->getDisplayName(),
    ...
    protected function doOmnipayOnsitePurchase($data, $paymentMethod = false)
    {
        ...
        if ($this->isGateway(GATEWAY_SAGE_PAY_DIRECT) || $this->isGateway(GATEWAY_SAGE_PAY_SERVER)) {
            $items = null;
        } elseif ($this->company()->send_item_details) {
        ...
        if ($response->isSuccessful() && $ref) {
            ...
            if ($this->invitation->invoice->company->isNinjaAccount()) {
    ...
    */
    ...
    public function createPayment($ref = false, $paymentMethod = null)
    {
        ...
        $payment->payment_date = $company->getDateTime()->format('Y-m-d');
        ...
        $companyKey = $invoice->company->account_key;

        if ($companyKey == env('NINJA_LICENSE_ACCOUNT_KEY')) {
            $this->createLicense($payment);
        // TODO move this code
            // enable pro plan for hosted users
        } elseif ($invoice->company->isNinjaAccount()) {
            ...
            if (! empty($plan)) {
                ...
                if (
                    ...
                    || DateTime::createFromFormat('Y-m-d', $company->companyPlan->plan_expires) <= date_create('-7 days')
                ) {
                    // Either this is a different plan, or the subscription expired more than a week ago
                    // Reset any grandfathering
                    $companyPlan->plan_started = date_create()->format('Y-m-d');
                }

                if (
                    $companyPlan->plan == $plan
                    && $companyPlan->plan_term == $term
                    && DateTime::createFromFormat('Y-m-d', $companyPlan->plan_expires) >= date_create()
                ) {
                    // This is a renewal; mark it paid as of when this term expires
                    $companyPlan->plan_paid = $companyPlan->plan_expires;
                } else {
                    $companyPlan->plan_paid = date_create()->format('Y-m-d');
                }

                $companyPlan->payment_id = $payment->id;
                $companyPlan->plan = $plan;
                $companyPlan->plan_term = $term;
                $companyPlan->plan_price = $price;
                $companyPlan->num_users = $numUsers;
                $companyPlan->plan_expires = DateTime::createFromFormat('Y-m-d', $company->companyPlan->plan_paid)
                    ->modify($term == PLAN_TERM_MONTHLY ? '+1 month' : '+1 year')->format('Y-m-d');

                if ($companyPlan->hasActivePromo()) {
                    $companyPlan->discount_expires = date_create()->modify('1 year')->format('Y-m-d');
                    $companyPlan->promo_expires = null;
                }
    ...
    public function rules()
    {
        ...
        if ($this->isGatewayType(GATEWAY_TYPE_CREDIT_CARD)) {
            ...
            if ($this->accountGateway->show_address) {
                ...
                if ($this->company()->requiresAddressState()) {
    ...
    public function completeOffsitePurchase($input)
    {
        ...
        if (Payment::whereCompanyPlanId($this->invitation->company_id)
    ...
    public function paymentLinks()
    {
        ...
        foreach ($this->gatewayTypes() as $gatewayTypeId) {
            ...
            if ($gatewayTypeId == GATEWAY_TYPE_CUSTOM1) {
                ...
                if ($custom = $this->company()->getLabel($gatewayTypeAlias)) {
                    $label = $custom;
                } else {
