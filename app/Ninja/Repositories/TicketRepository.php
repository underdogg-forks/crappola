...
class TicketRepository extends BaseRepository
{
    ...
    public function find($filter = null, $userId = false, $entityType = ENTITY_TICKET)
    {
        $query = DB::table('tickets')
            ...
            ->leftJoin('ticket_comments', function ($join): void {
    ...
    public function save($input, $ticket = false, $harvestedUser = false)
    {
        ...
        if (Auth::user()) {
            ...
            $user = User::where('id', '=', $contact->company->company_ticket_settings->ticket_master_id)->first();
        ...
        if (! $ticket) {
            if ($contact) {
                ...
                $ticket->ticket_number = Ticket::getNextTicketNumber($contact->company->id);
                $ticket->priority_id = TICKET_PRIORITY_LOW;
            } else {
                $ticket = Ticket::createNew($user);
                $ticket->ticket_number = Ticket::getNextTicketNumber($user->company->id);
            }
        }
        ...
        if (! empty($input['document_ids'])) {
            ...
            foreach ($document_ids as $document_id) {
                $document = Document::scope($document_id, $ticket->company_id)->first();
    ...
    private function createTicketInvite($ticket, $contactId, $user): void
    {
