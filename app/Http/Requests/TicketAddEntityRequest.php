...
class TicketAddEntityRequest extends EntityRequest
{
    ...
    public function addEntity(): TicketRelation
    {
        ...
        $entityId = $entityModel::getPortalPrivateId(request()->entity_id, request()->company_id);
        ...
        $str = self::buildEntityUrl($linkEntity, request()->entity_id, request()->company_id);
    ...
    private static function buildEntityUrl($entityType, $publicId, $companyId): string
    {
        ...
        $className = '\App\Models\\' . ucfirst($entityType);
        $entityModel = new $className();
        $entity = $entityModel::scope($publicId, $companyId)->first();
