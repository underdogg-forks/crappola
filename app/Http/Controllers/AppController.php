...
class AppController extends BaseController
{
    ...
    public function __construct(AccountRepository $companyRepo, Mailer $mailer, EmailService $emailService)
    {
        //parent::__construct();
    ...
    public function updateSetup()
    {
        ...
        if ($mail) {
            $prefix = '';
            if (($user = auth()->user()) && company::count() > 1) {
                $prefix = $user->company_id . '_';
            }
    ...
    public function stats()
    {
        ...
        if (Utils::getResllerType() == RESELLER_REVENUE_SHARE) {
            $data = DB::table('companies')
                ...
                ->where('companies.account_key', '=', NINJA_ACCOUNT_KEY)
