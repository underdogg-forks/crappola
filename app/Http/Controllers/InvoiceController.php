...
class InvoiceController extends BaseController
{
    ...
    public function getRecurringDatatable(Request $request, $clientPublicId = null)
    {
    ...
    public function getDatatable(Request $request, $clientPublicId = null)
    {
        $companyId = Auth::user()->company_id;
        $search = $request->get('sSearch');
    ...
    /**
     * @return array{data: mixed, company: mixed, products: mixed, taxRateOptions: mixed, sizes: mixed, invoiceDesigns: mixed, invoiceFonts: mixed, frequencies: mixed, recurringDueDates: array<mixed, array{value: string, data-num?: int, class: string}>&mixed[], recurringHelp: string, recurringDueDateHelp: string, invoiceLabels: mixed, tasks: mixed, expenseCurrencyId: mixed, expenses: mixed}
     */
    ...
    private function emailInvoice($invoice)
    {
        ...
        if (filter_var($request->get('save_as_default'), FILTER_VALIDATE_BOOLEAN)) {
            $company = Auth::user()->company;
            $company->setTemplateDefaults($request->get('template_type'), $template['subject'], $template['body']);
        }
    ...
    public function edit(InvoiceRequest $request, $publicId, $clone = false)
    {
        ...
        $invoice->recurring_due_date = $invoice->due_at; // Keep in SQL form
                ...
                foreach ($invoice->invitations as $invitation) {
                    foreach ($client->contacts as $contact) {
                        if ($invitation->contact_id == $contact->id) {
                            $hasPassword = $company->isClientPortalPasswordEnabled() && $contact->password;
    ...
    {
        $invoice = $request->entity();
        $invoice->load('user', 'invoice_items', 'documents', 'expenses', 'expenses.documents', 'company.country', 'client.contacts', 'client.country', 'client.shipping_country');
        $invoice->invoice_date = Utils::fromSqlDate($invoice->invoice_date);
        $invoice->due_at = Utils::fromSqlDate($invoice->due_at);
        $invoice->features = [
            'customize_invoice_design' => Auth::user()->hasFeature(FEATURE_CUSTOMIZE_INVOICE_DESIGN),
