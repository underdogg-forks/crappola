...
class TicketService extends BaseService
{
    ...
    {
        $clients = Client::scope()->with('contacts')->whereHas('contacts', function ($query) use ($email): void {
    ...
    public function getRelationCollection(Request $request)
    {
        ...
        $query = $entityModel::scope($client_public_id, $request->company_id);
        ...
        if ($client_public_id > 0) {
            $clientPrivateId = Client::getPortalPrivateId($client_public_id, $request->company_id);
